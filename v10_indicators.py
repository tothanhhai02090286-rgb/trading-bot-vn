...
def add_indicators(df):
    ...
    # ========== THÊM CÁC CỘT CHO RSI DIVERGENCE ==========
    df["Close_5"] = close.shift(5)
    df["Close_20"] = close.shift(20)
    df["RSI_5"] = df["RSI"].shift(5)
    df["RSI_20"] = df["RSI"].shift(20)
    df["RSI_MA5"] = df["RSI"].rolling(5).mean()
    # ====================================================

    # ========== ON-BALANCE VOLUME (OBV) ==========
    price_change = close.diff()
    obv = (volume * (~price_change.le(0) * 2 - 1)).cumsum()
    df["OBV"] = obv
    df["OBV_MA20"] = obv.rolling(20).mean()
    close_shift20 = close.shift(20)
    obv_shift20 = obv.shift(20)
    df["OBV_Bullish_Div"] = (close < close_shift20) & (obv > obv_shift20)
    df["OBV_Bearish_Div"] = (close > close_shift20) & (obv < obv_shift20)
    # ==============================================
