...
def repair_mojibake(text):
    ...
    Xử lý trường hợp UTF-8 bị đọc nhầm thành Latin-1 hoặc CP1258.
    """
    if not isinstance(text, str) or pd.isna(text):
        return text
    
    # Lưu original để fallback
    original = text
    
    # Method 1: UTF-8 bytes bị đọc sai thành Latin-1
    try:
        # Bước 1: ép về bytes với encoding latin1
        text_bytes = text.encode('latin1')
        # Bước 2: decode lại với utf-8
    ...
    # Method 2: Thử với cp1252 (Windows)
    try:
        text_bytes = text.encode('cp1252')
        fixed = text_bytes.decode('utf-8')
        if any(c in fixed for c in 'àáảãạâầấẩẫậăằắẳẵặ'):
            return fixed
    except:
        pass
    
    # Method 3: Fix trực tiếp các pattern phổ biến
    fixes = {
        'Ã¡': 'á', 'Ã ': 'à', 'Ã£': 'ã', 'áº¡': 'ạ',
        ...
        'Ä': 'đ', 'â': '-', 'â': '"', 'â': '"',
        'MÃ£': 'Mã', 'NgÃ y': 'Ngày', 'GiÃ¡': 'Giá',
        'Chiáº¿n lÆ°á»£c': 'Chiến lược', 'LÃ£i/Lá»': 'Lãi/Lỗ'
    }
    ...
    # Nếu có thay đổi thì return
    if result != text:
        return result
    
    # Không fix được, return original
    return original

def fix_all_vietnamese_columns(df):
    """
    Fix tất cả column names và string columns bị vỡ font
    """
    ...
    for col in df.columns:
        fixed = repair_mojibake(str(col))
        # Chuẩn hóa: bỏ khoảng trắng thừa
        fixed = fixed.strip().replace(' ', '_')
        new_cols.append(fixed)
    df.columns = new_cols
    
    # 2. Fix các cột kiểu string
    for col in df.columns:
        if df[col].dtype == 'object':
            df[col] = df[col].apply(lambda x: repair_mojibake(x) if isinstance(x, str) else x)
...
def safe_read_csv(path):
    ...
    for enc in ["utf-8-sig", "utf-8", "cp1258", "latin1"]:
        try:
            ...
            # Apply mojibake repair first
            df = fix_all_vietnamese_columns(df)
            # Then apply existing column mapping
