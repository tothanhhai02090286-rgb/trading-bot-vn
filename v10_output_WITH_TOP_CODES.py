...
# ============================================================
# TOP CODES BY PROVEN PATTERN (NEW)
# - Keeps old TOP PROVEN PATTERNS unchanged
# - Adds actionable tickers that match proven historical patterns
# ============================================================

def build_top_codes_by_proven_pattern(signal_df, pattern_df, limit=20):
    ...
    pat = normalize_output_columns(pattern_df.copy())

    pattern_col = None
    for c in ["Pattern", "Pattern Key"]:
        if c in pat.columns:
            pattern_col = c
            break

    signal_pattern_col = None
    for c in ["Pattern", "Pattern Key"]:
        if c in sig.columns:
            signal_pattern_col = c
            break

    if pattern_col is None or signal_pattern_col is None:
        return pd.DataFrame()

    oos_pct_col = None
    for c in ["OOS%", "OOS Win Probability", "OOS Win Rate"]:
        if c in pat.columns:
            oos_pct_col = c
            break

    oos_n_col = None
    for c in ["OOSN", "OOS N", "OOS Samples"]:
        if c in pat.columns:
            oos_n_col = c
            break
    ...
    # - OOS >= 80 with at least 5 samples
    # - or OOS >= 70 with at least 8 samples
    strong_pat = pat[
        ((pat[oos_pct_col] >= 80) & (pat[oos_n_col] >= 5)) |
        ((pat[oos_pct_col] >= 70) & (pat[oos_n_col] >= 8))
    ].copy()

    if strong_pat.empty:
        return pd.DataFrame()

    keep_pat_cols = [pattern_col, oos_pct_col, oos_n_col]
    for c in ["Trust", "Note", "Source", "Avg+5D", "Avg+10D"]:
        if c in strong_pat.columns:
            keep_pat_cols.append(c)

    strong_pat = strong_pat[keep_pat_cols].drop_duplicates(subset=[pattern_col], keep="first")
    ...
    rename = {
        ...
        "AI Confidence": "AI",
        "Win Probability": "Win%",
        oos_pct_col: "Pattern OOS%",
        oos_n_col: "Pattern OOSN",
        pattern_col: "Pattern",
    }
    merged = merged.rename(columns={k: v for k, v in rename.items() if k in merged.columns})

    for col in ["Rec", "Strategy", "Risk Status"]:
        if col in merged.columns:
            merged[col] = merged[col].astype(str).apply(clean_display_na)

    if "Pattern" in merged.columns:
        merged["Pattern"] = merged["Pattern"].astype(str).apply(lambda x: clean_ascii_text(x, 160))

    for col in [
        "Pattern OOS%", "Pattern OOSN", "Score", "AI", "Win%",
        "RSI", "RS20", "Price", "Volume Ratio", "ATR %"
    ]:
        if col in merged.columns:
            merged[col] = pd.to_numeric(merged[col], errors="coerce")

    sort_cols = [c for c in ["Pattern OOS%", "Pattern OOSN", "AI", "Score"] if c in merged.columns]
    if sort_cols:
        merged = merged.sort_values(sort_cols, ascending=False)

    preferred = [
        "Date", "Code", "Price", "Rec",
        "Pattern OOS%", "Pattern OOSN", "Trust", "Note",
        "Score", "AI", "Win%",
        "Strategy", "RSI", "RS20", "Volume Ratio", "ATR %",
