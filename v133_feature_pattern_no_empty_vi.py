# -*- coding: utf-8 -*-
# ============================================================
# V13.3 FEATURE PATTERN PRO - KHONG TRA VE RONG
# - Feature-based pattern engine
# - Match theo trong so rieng Momentum / Bottom
# - Co fallback: neu khong dat nguong van lay mau gan nhat
# - Dashboard tieng Viet de doc
# ============================================================
...
VI_MAP = {
    "UPTREND": "Thị trường tăng",
    "DOWNTREND": "Thị trường giảm",
    "SIDEWAY": "Đi ngang",
    "MOMENTUM": "Đà tăng",
    "MOMENTUM_WATCH": "Theo dõi đà tăng",
    "BOTTOM": "Bắt đáy",
    "BOTTOM_WATCH": "Theo dõi đáy",
    "WATCH": "Theo dõi",
    "BUY NOW": "Mua ngay",
    "WAIT": "Chờ",
    "WATCHLIST": "Theo dõi",
    ...
    "RSI_LOW": "RSI thấp",
    "RSI_WEAK": "RSI yếu",
    "RSI_MID": "RSI trung bình",
    "RSI_MID_HIGH": "RSI khá cao",
    "RSI_HIGH": "RSI cao",
    "RSI_OVERHEAT": "RSI quá nóng",
    "RS20_BAD": "RS20 yếu",
    "RS20_SOFT": "RS20 hơi yếu",
    "RS20_OK": "RS20 ổn",
    "RS20_STRONG": "RS20 mạnh",
    "RS20_LEADER": "RS20 dẫn dắt",
    "VOL_LOW": "Thanh khoản thấp",
    "VOL_OK": "Thanh khoản ổn",
    "VOL_STRONG": "Thanh khoản mạnh",
    "ATR_LOW": "Biến động thấp",
    "ATR_OK": "Biến động ổn",
    "ATR_HIGH": "Biến động cao",
    "ABOVE_MA20": "Giá trên MA20",
    "BELOW_MA20": "Giá dưới MA20",
    "FAR_MA20": "Giá xa MA20",
}


def _norm_token(t):
    t = str(t).strip().upper()
    return TOKEN_MAP.get(t, t)
...
def current_row_to_features(row):
    # Neu co pattern goc thi parse truoc
    for c in ["Deep Pattern", "Pattern", "Pattern Key"]:
        if c in row.index and _txt(row.get(c)):
            f = pattern_to_features(row.get(c))
            if not f["RSI"]:
                f["RSI"] = _bucket_rsi(row.get("RSI"))
            if not f["RS20"]:
                f["RS20"] = _bucket_rs20(row.get("RS20"))
            if not f["Volume"]:
                f["Volume"] = _bucket_volume(row.get("Volume Ratio"))
            if not f["ATR"]:
                f["ATR"] = _bucket_atr(row.get("ATR %"))
            if not f["MA20"]:
                f["MA20"] = _bucket_ma20(row)
            # Neu pattern goc thieu regime/strategy/action thi bo sung
            if not f["Regime"]:
                f["Regime"] = _infer_regime(row)
            if not f["Strategy"]:
                f["Strategy"] = _infer_strategy(row)
            if not f["Action"]:
                f["Action"] = _infer_action(row)
            return f
...
def _infer_regime(row):
    ...
    if "UPTREND" in regime or "TANG" in regime or "TĂNG" in regime:
        return "UPTREND"
    if "DOWN" in regime or "GIAM" in regime or "GIẢM" in regime:
        return "DOWNTREND"
    return "SIDEWAY"
...
def _infer_strategy(row):
    ...
    if "BOTTOM" in st or "ĐÁY" in st or "DAY" in st:
        return "BOTTOM_WATCH" if "WATCH" in st or "THEO" in st else "BOTTOM"
    if "MOMENTUM" in st or "ĐÀ" in st or "DA" in st:
        return "MOMENTUM_WATCH" if "WATCH" in st or "THEO" in st else "MOMENTUM"
    if "WATCH" in st or "THEO" in st:
        return "WATCH"
    return st


def _infer_action(row):
    action = _up(row.get("Action", row.get("Rec", row.get("Final Action", row.get("Hanh dong", row.get("Hành động", ""))))))
    if "BUY" in action or "MUA" in action:
        return "BUY NOW"
    if "WAIT" in action or "CHO" in action or "CHỜ" in action:
        return "WAIT"
    if "WATCH" in action or "THEO" in action:
        return "WATCHLIST"
    if "SKIP" in action or "BO" in action or "BỎ" in action:
        return "SKIP"
    return ""
...
def _sim(field, a, b):
    ...
    if field == "Regime":
        if set([a, b]) == set(["UPTREND", "SIDEWAY"]):
            return 0.5
        if set([a, b]) == set(["DOWNTREND", "SIDEWAY"]):
            return 0.4
        return 0.0

    if field == "Strategy":
        if "MOMENTUM" in a and "MOMENTUM" in b:
            return 0.6
        if "BOTTOM" in a and "BOTTOM" in b:
            return 0.6
        return 0.0
    ...
    if field == "MA20":
        if set([a, b]) == set(["ABOVE_MA20", "FAR_MA20"]):
            return 0.6
        return 0.0

    if field == "Action":
        if a in ["BUY NOW", "WAIT"] and b in ["BUY NOW", "WAIT"]:
            return 0.5
        if a in ["WATCHLIST", "WAIT"] and b in ["WATCHLIST", "WAIT"]:
            return 0.5
        return 0.0
...
def weighted_feature_match(current_f, hist_f):
    ...
    for field, w in weights.items():
        ...
        got += pts
        if s > 0:
            details.append(f"{field}: {round(pts, 1)}/{w}")
...
def _best_match_no_empty(current_f, records):
    """
    Luon tra ve mau gan nhat neu records ton tai.
    Khong dung nguong de tranh dashboard rong.
    """
    best = None
    for rec in records:
        ...
        item["_rank"] = (
            pct,
            _num(item.get("Số lần test"), 0),
            _num(item.get("Lợi TB T+5 %"), -999),
            _num(item.get("Tỷ lệ thắng %"), -999),
        )
...
def _rank_history(n, wr, a5, pct):
    ...
    if pct >= 75 and n >= 5 and wr >= 70 and a5 >= 3:
        return "MẠNH"
    if pct >= 60 and n >= 5 and wr >= 65 and a5 >= 1:
        return "DÙNG ĐƯỢC"
    if pct >= 40:
        return "MATCH YẾU - CHỈ THAM KHẢO"
    return "MATCH RẤT YẾU - KHÔNG ƯU TIÊN"
...
def _score(n, wr, a2, a5, a10, pct):
    ...
    if not pd.isna(a5):
        s += max(min(a5 * 3, 25), -20)
    if not pd.isna(a2):
        s += max(min(a2 * 1.5, 10), -10)
    if not pd.isna(a10):
        s += max(min(a10 * 0.8, 8), -8)
    return round(max(min(s, 100), 0), 1)


def _ket_luan(row):
    risk = _up(row.get("Risk"))
    rank = _txt(row.get("Độ tin cậy lịch sử"))
    pct = _num(row.get("Mức khớp mẫu %"), 0)
    score = _num(row.get("Score"), 0)
    ai = _num(row.get("AI"), 0)

    if risk == "FAIL":
        return "BỎ QUA - RISK FAIL"
    if rank in ["RẤT MẠNH", "MẠNH"] and score >= 75 and ai >= 75:
        return "ƯU TIÊN THEO DÕI / MUA THĂM DÒ"
    if rank == "DÙNG ĐƯỢC":
        return "THEO DÕI - CÓ LỊCH SỬ HỖ TRỢ"
    if "MATCH YẾU" in rank:
        return "CHỈ THAM KHẢO - KHÔNG ƯU TIÊN"
    if pct < 40:
...
def build_v133_feature_pattern_view_vi(current_df, pattern_stats_df, limit=60):
    try:
        ...
        for _, r in df.iterrows():
            ...
            best = _best_match_no_empty(cur_f, records)
            ...
            a2 = best["Lợi TB T+2 %"]
            a5 = best["Lợi TB T+5 %"]
            a10 = best["Lợi TB T+10 %"]
            pct = best["Mức khớp mẫu %"]
            ...
            row = {
                ...
                "Feature lịch sử gần nhất": _feature_vi(best.get("Feature lịch sử", {})),
                "Chi tiết khớp": best.get("Chi tiết khớp", ""),
                "Pattern hiện tại": features_to_key(cur_f),
                "Pattern lịch sử gần nhất": best.get("Pattern lịch sử", ""),
            }
            row["Kết luận V13.3"] = _ket_luan(row)
            row["Lý do"] = f"mẫu gần nhất khớp {pct}%; lịch sử {rank}; test {n}; win {'' if pd.isna(wr) else round(wr,1)}%; T+5 {'' if pd.isna(a5) else round(a5,2)}%"
            rows.append(row)

        out = pd.DataFrame(rows)
        order = {"RẤT MẠNH": 1, "MẠNH": 2, "DÙNG ĐƯỢC": 3, "MATCH YẾU - CHỈ THAM KHẢO": 5, "MATCH RẤT YẾU - KHÔNG ƯU TIÊN": 9, "CHƯA ĐỦ DỮ LIỆU": 10}
        ...
        out = out.sort_values(["_rank", "Điểm lịch sử", "Mức khớp mẫu %", "AI", "Score"], ascending=[True, False, False, False, False]).drop(columns=["_rank"])
        return out.replace({np.nan: ""}).head(limit)

    except Exception as e:
        return pd.DataFrame([{"Trạng thái": "Lỗi V13.3 feature pattern", "Chi tiết": repr(e)}])


def build_v133_top_picks_vi(v133_df, limit=8):
    try:
        if v133_df is None or v133_df.empty or "Độ tin cậy lịch sử" not in v133_df.columns:
            return pd.DataFrame([{"Trạng thái": "Không có dữ liệu V13.3"}])

        df = v133_df.copy()
        # Không trả rỗng: ưu tiên mạnh, nếu không có thì lấy top match yếu để tham khảo.
        good = df[df["Độ tin cậy lịch sử"].astype(str).isin(["RẤT MẠNH", "MẠNH", "DÙNG ĐƯỢC"])].copy()
        if good.empty:
            good = df.copy()
        ...
        cols = ["Mã", "Giá", "Hành động hiện tại", "Strategy", "Kết luận V13.3", "Độ tin cậy lịch sử", "Mức khớp mẫu %", "Số lần test", "Tỷ lệ thắng %", "Lợi TB T+5 %", "Điểm lịch sử", "AI", "Risk", "Lý do"]
        return good[[c for c in cols if c in good.columns]].head(limit).replace({np.nan: ""})

    except Exception as e:
        return pd.DataFrame([{"Trạng thái": "Lỗi top V13.3", "Chi tiết": repr(e)}])
