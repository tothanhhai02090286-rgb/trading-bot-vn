...
def advanced_ai_filter(row, market_ret20=0):
    ...
    obv_ma20 = safe_float(row.get("OBV_MA20"), 0)
    obv_bull = bool(row.get("OBV_Bullish_Div", False))
    obv_bear = bool(row.get("OBV_Bearish_Div", False))
    
    # So sánh OBV với đường MA20
    if obv > obv_ma20 and obv_ma20 > 0:
        confidence += 5
        reasons.append("OBV trên MA20 - dòng tiền tích lũy")
    elif obv < obv_ma20 and obv_ma20 > 0:
        confidence -= 5
        warnings.append("OBV dưới MA20 - dòng tiền rút ra")
    
    # OBV phân kỳ tăng (bullish divergence)
    if obv_bull:
        confidence += 12
        reasons.append("OBV phân kỳ tăng - giá giảm nhưng OBV tăng, tích lũy đáy")
    
    # OBV phân kỳ giảm (bearish divergence)
    if obv_bear:
        confidence -= 15
        warnings.append("OBV phân kỳ giảm - giá tăng nhưng OBV giảm, phân phối đỉnh")
    # ==============================================
