...
Nâng cấp từ intraday_alert_bot.py:
- Đọc watchlist từ RAW_URL / GITHUB_RAW_WATCHLIST_URL
- Startup log Telegram: RAW_URL, rows, tickers
- VWAP filter
- Breakout confirmation
- Pullback entry
- Volume spike intraday
- Anti false breakout
- Cooldown alert
- Fallback về logic V17 cũ nếu không lấy được intraday bars

Biến môi trường Render nên có:
- TELEGRAM_TOKEN
- TELEGRAM_CHAT_ID
- RAW_URL hoặc GITHUB_RAW_WATCHLIST_URL
...
- MARKET_END=14:50
- TZ=Asia/Ho_Chi_Minh
- V18_ENABLE_EXECUTION=1
- V18_MIN_VOLUME_RATIO=1.5
- V18_BREAKOUT_BUFFER_PCT=0.3
- V18_PULLBACK_TOLERANCE_PCT=0.7
- V18_VWAP_TOLERANCE_PCT=0.2
- V18_ALERT_COOLDOWN_MIN=30
"""
...
def get_current_price_vnstock(symbol: str) -> Optional[float]:
    try:
        ...
        if not df.empty:
            return float(df["price_norm"].iloc[-1])
...
def build_intraday_snapshot(symbol: str) -> Dict[str, Any]:
    ...
    price = float(bars["price_norm"].iloc[-1])
    prev = float(bars["price_norm"].iloc[-2]) if len(bars) >= 2 else None

    vol = pd.to_numeric(bars["volume_norm"], errors="coerce").fillna(0)
    px = pd.to_numeric(bars["price_norm"], errors="coerce")

    total_vol = float(vol.sum())
    if total_vol > 0:
        vwap = float((px * vol).sum() / total_vol)
    else:
        vwap = float(px.mean())
    ...
    session_high_prev = float(prev_prices.max()) if len(prev_prices) else price
    session_low = float(px.min()) if len(px) else price
...
def detect_v18_execution_signal(row: pd.Series, snap: Dict[str, Any]) -> Optional[str]:
    ...
    prev_price = snap.get("prev_price")
    vwap = snap.get("vwap")
    volume_ratio = snap.get("volume_ratio")
    session_high_prev = snap.get("session_high_prev")
    ...
    # 1) Ưu tiên tín hiệu mạnh nhất: reclaim VWAP + volume.
    if vwap_reclaim and vol_ok:
        return "🔥 VWAP RECLAIM + VOLUME"

    # 2) Breakout xác nhận: vượt đỉnh intraday trước đó hoặc vượt buy_high/ref.
    breakout_levels = []
    ...
    # 3) Pullback đẹp: giá nằm trong buy zone và không mất VWAP quá sâu.
    ...
    # 4) Legacy momentum nhưng yêu cầu không quá yếu so với VWAP.
...
def build_alert(row: pd.Series, snap: Dict[str, Any], alert_type: str) -> str:
    ...
    return (
        ...
        f"Nhóm: <b>{group}</b>\n"
        f"Giá hiện tại: <b>{float(current_price):.2f}</b>\n"
        f"VWAP: <b>{vwap:.2f}</b>\n" if vwap is not None else
        f"{emoji} <b>{symbol}</b> - <b>{alert_type}</b>\n"
        f"Nhóm: <b>{group}</b>\n"
        f"Giá hiện tại: <b>{float(current_price):.2f}</b>\n"
    ) + (
        f"Volume ratio: <b>{volume_ratio:.2f}</b>\n" if volume_ratio is not None else ""
    ) + (
