...
try:
    ...
    V202_CONTEXT_FILTER_ON = True
except Exception as e:
    print("WARN V20.2 context filter import failed:", repr(e), flush=True)
    V202_CONTEXT_FILTER_ON = False
try:
...
def load_watchlist() -> pd.DataFrame:
    if RAW_URL:
        ...
        try:
            url = RAW_URL
            use_cache_bust = os.getenv("RAW_URL_CACHE_BUST", "1") == "1"
            if use_cache_bust:
                sep = "&" if "?" in url else "?"
...
def build_alert(row: pd.Series, snap: Dict[str, Any], signal_pack: Dict[str, Any]) -> str:
    ...
    v202 = signal_pack.get("v202_context", {})
    if v202:
        msg += (
            f"\n<b>V20.2 Context Filter:</b>\n"
            f"Action: <b>{v202.get('v202_action', '')}</b>\n"
            f"Penalty: <b>{v202.get('v202_penalty', '')}</b>\n"
            f"Lý do: {' | '.join(v202.get('v202_reasons', []))}\n"
        )

    msg += f"Time: {_now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        if signal_pack:
            ...
            if V202_CONTEXT_FILTER_ON:
                v202 = evaluate_v202_realtime_context(
                    ...
                    stock_ret20=None,
                    market_ret5=None,
                    market_dist_ma20_pct=None,
                    market_context="",
                )

                old_rec = signal_pack["recommendation"]["final_recommendation"]
                new_rec = apply_v202_rule_cap(old_rec, v202)

                signal_pack["recommendation"]["final_recommendation"] = new_rec
                signal_pack["v202_context"] = v202

                if new_rec != old_rec:
                    signal_pack["reasons"].append(
                        f"⚠ V20.2 hạ từ {old_rec} xuống {new_rec}: "
                        + " | ".join(v202.get("v202_reasons", []))
                    )

            key = f"{symbol}:{signal}"

            if key not in set(state.get("sent", [])) or _cooldown_ok(state, symbol, signal, quality["cooldown_min"]):
                msg = build_alert(row, snap, signal_pack)
                send_telegram(msg)
                ...
                log_entry_alert(
                    ...
                    buy_zone_high=high,
                    stoploss=stop,
                    decision_mode=fields.get("decision_mode", ""),
                    market_regime=fields.get("regime_strength", ""),
                    reason=" | ".join(signal_pack.get("reasons", [])),
                )
...
def startup_message(df: pd.DataFrame) -> str:
    ...
    # Ưu tiên schema mới từ V17.1. Nếu RAW_URL vẫn đang trỏ schema cũ,
    # tự suy luận để Telegram không hiện UNKNOWN / {}.
    if not df.empty and "Decision Mode" in df.columns:
        decision_modes = sorted(set(df["Decision Mode"].dropna().astype(str).tolist()))
    else:
        decision_modes = ["LEGACY WATCH MODE"]

    if not df.empty and "Final Decision" in df.columns:
        final_counts = df["Final Decision"].dropna().astype(str).value_counts().to_dict()
    elif not df.empty and "Hành động" in df.columns:
        final_counts = df["Hành động"].dropna().astype(str).value_counts().to_dict()
    elif not df.empty and "Nhóm realtime" in df.columns:
        final_counts = df["Nhóm realtime"].dropna().astype(str).value_counts().to_dict()
    else:
        final_counts = {}
    ...
    return (
        ...
        f"Decision Mode: <b>{', '.join(decision_modes) if decision_modes else 'LEGACY WATCH MODE'}</b>\n"
