...
from v11_pattern_match_vi import build_pattern_match_view_vi, build_pattern_match_top_picks_vi
...
def main():
    ...
    # V11 PATTERN MATCH VIEW VI - SAFE
    try:
        pattern_match_view = build_pattern_match_view_vi(combined, back_wf_stats_disp, limit=40)
        pattern_match_top_view = build_pattern_match_top_picks_vi(pattern_match_view, limit=8)
        print("PATTERN MATCH VIEW OK")
    except Exception as e:
        print("WARN: pattern match vi error:", repr(e))
        pattern_match_view = pd.DataFrame([{
            "Trang thai": "Pattern match loi nhung da bo qua de khong crash",
            "Chi tiet": repr(e)
        }])
        pattern_match_top_view = pd.DataFrame()
    ...
    try:
        pattern_match_html = pattern_match_view.to_html(index=False, escape=True)
    except Exception:
        pattern_match_html = ""
    try:
        pattern_match_top_html = pattern_match_top_view.to_html(index=False, escape=True)
    except Exception:
        pattern_match_top_html = ""
    except Exception:
...
<h3>V11 - TOP MA KHOP MAU TOT</h3>
{pattern_match_top_html}

<h3>V11 - MA DANG KHOP MAU LICH SU NAO</h3>
{pattern_match_html}
