...
def main():
    ...
    # PRO MAX FIX:
    # Ensure current signals have Pattern Key so they can be matched with proven historical patterns.
    try:
        if "Pattern Key" not in combined.columns:
            combined["Pattern Key"] = combined.apply(
                lambda r: make_pattern_key(r, current_market_regime),
                axis=1
            )
        else:
            missing_pk = combined["Pattern Key"].isna() | (combined["Pattern Key"].astype(str).str.strip() == "")
            if missing_pk.any():
                combined.loc[missing_pk, "Pattern Key"] = combined.loc[missing_pk].apply(
                    lambda r: make_pattern_key(r, current_market_regime),
                    axis=1
                )
    except Exception as e:
        print("WARN: cannot rebuild Pattern Key for current signals:", repr(e))

    # PRO MAX: map proven patterns back to today's tickers by holding horizon
    try:
        top_codes_t2_view = build_top_codes_by_proven_pattern(
            combined,
            back_wf_stats_disp,
            mode="T2",
            limit=20
        ...
        print("WARN: TOP CODES T+2 error:", repr(e))
        top_codes_t2_view = pd.DataFrame()

    try:
        top_codes_t5_view = build_top_codes_by_proven_pattern(
            combined,
            back_wf_stats_disp,
            mode="T5",
            limit=20
        ...
        print("WARN: TOP CODES T+5 error:", repr(e))
        top_codes_t5_view = pd.DataFrame()

    try:
        pattern_codes_map_view = build_pattern_to_codes_map(
            combined,
            back_wf_stats_disp,
            mode="ALL",
            limit=20
        ...
        print("WARN: PATTERN TO CODES MAP error:", repr(e))
