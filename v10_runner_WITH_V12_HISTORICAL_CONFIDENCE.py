...
from v12_historical_confidence_vi import build_do_tin_cay_lich_su_vi, build_top_quyet_dinh_lich_su_vi
...
def main():
    ...
    # V12 DO TIN CAY LICH SU - VIET HOA CHUAN
    try:
        do_tin_cay_lich_su_view = build_do_tin_cay_lich_su_vi(combined, back_wf_stats_disp, limit=50)
        top_quyet_dinh_lich_su_view = build_top_quyet_dinh_lich_su_vi(do_tin_cay_lich_su_view, limit=10)
        print("V12 DO TIN CAY LICH SU OK")
    except Exception as e:
        print("WARN: V12 do tin cay lich su error:", repr(e))
        do_tin_cay_lich_su_view = pd.DataFrame([{"Trạng thái":"Lỗi độ tin cậy lịch sử nhưng đã bỏ qua để không crash","Chi tiết":repr(e)}])
        top_quyet_dinh_lich_su_view = pd.DataFrame()
    ...
    try:
        do_tin_cay_lich_su_html = do_tin_cay_lich_su_view.to_html(index=False, escape=True)
    except Exception:
        do_tin_cay_lich_su_html = ""
    try:
        top_quyet_dinh_lich_su_html = top_quyet_dinh_lich_su_view.to_html(index=False, escape=True)
    except Exception:
        top_quyet_dinh_lich_su_html = ""
...
<h3>V12 - TOP QUYET DINH THEO LICH SU</h3>
{top_quyet_dinh_lich_su_html}

<h3>V12 - DO TIN CAY LICH SU THEO TUNG MA</h3>
{do_tin_cay_lich_su_html}
