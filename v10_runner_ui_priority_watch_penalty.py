...
def _ui_downgrade_buy_now_when_t2_t5_negative(df):
    ...
    If current action is BUY NOW and both historical T+2/T+5 returns are negative,
    show it as WATCHLIST. No other rows or logic are changed.
    """
    if df is None or getattr(df, "empty", True):
        return df
    out = df.copy()
    ...
    if t2_col is None or t5_col is None:
        return out

    action_text = out[action_col].astype(str).str.upper()
    t2 = pd.to_numeric(out[t2_col], errors="coerce")
    t5 = pd.to_numeric(out[t5_col], errors="coerce")
    mask = action_text.str.contains("BUY NOW", na=False) & (t2 < 0) & (t5 < 0)
...
<div class="ui-note">Bản này chỉ thêm bảng TOP ở đầu dashboard, làm nổi bật tên mã theo màu xanh/đỏ, và hạ BUY NOW xuống WATCHLIST nếu T+2 và T+5 cùng âm. Logic lọc gốc không đổi.</div>
