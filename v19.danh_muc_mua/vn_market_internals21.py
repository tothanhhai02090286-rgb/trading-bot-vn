...
def _distribution_days(cache_dir: str) -> Dict[str, Any]:
    ...
    if h.empty or len(h) < 25:
        return {
            ...
            "recent_dates": "", "latest_date": "", "latest_note": ""
        }
    ...
    if events:
        ...
        value_bn = liq.get("value_bn", "")
        avg20_bn = liq.get("avg20_bn", "")
        vs20_pct = liq.get("vs20_pct", "")
        liq_text = ""
        if value_bn != "":
            liq_text = f" | GTGD 138 mã: {value_bn:,.0f} tỷ; TB20: {avg20_bn:,.0f} tỷ; bằng {vs20_pct}% TB20"
