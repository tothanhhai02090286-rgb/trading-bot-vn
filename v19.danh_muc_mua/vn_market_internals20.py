...
def _participation_from_index(cache_dir: str) -> Dict[str, Any]:
    ...
    if h.empty or len(h) < 21:
        return {"pct": "", "label": "⚪ Không đủ dữ liệu", "last_update": ""}
    ...
    return {"pct": pct, "label": label, "last_update": _fmt_latest_date(h)}


def _distribution_days(cache_dir: str) -> Dict[str, Any]:
    h = _load_history("VNINDEX", cache_dir)
    if h.empty or len(h) < 25:
        return {"count": "", "label": "⚪ Không đủ dữ liệu", "last_update": ""}
    ...
    for i in range(start, len(h)):
        ...
        if ret <= -0.6 and vol_base > 0 and float(vol.iloc[i]) >= vol_base * 1.10:
            cnt += 1
    if cnt >= 5:
    ...
    return {"count": cnt, "label": label, "last_update": _fmt_latest_date(h)}
