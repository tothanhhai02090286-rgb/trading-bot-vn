...
def _sector_conviction_label(score: float) -> str:
    """PHASE 18.1 — nhãn độ tin cậy ngành, dễ đọc cho Telegram."""
    score = _to_num(score, 0.0)
    if score >= 85:
        return "🟢 MUA ĐƯỢC / ƯU TIÊN RẤT CAO"
    if score >= 70:
        return "🟢 WATCHLIST ƯU TIÊN"
    if score >= 55:
        return "🟡 THEO DÕI"
    if score >= 40:
        return "🟠 THĂM DÒ"
    return "🔴 TRÁNH / KHÔNG ƯU TIÊN"


def _leader_score_from_count(count: Any) -> float:
    """Quy đổi số leader thật trong ngành sang điểm 0-100.

    0 leader = 0, 1 = 25, 2 = 45, 3 = 65, 4 = 80, >=5 = 100.
    Mục tiêu: tránh ngành nhỏ có Flow cao nhưng không có cổ phiếu dẫn dắt thật.
    """
    c = int(_to_num(count, 0))
    if c <= 0:
        return 0.0
    if c == 1:
        return 25.0
...
def _sector_conviction_score(r: Dict[str, Any]) -> float:
    """PHASE 18.1 — Sector Conviction Score.

    Conviction = mức độ đáng tin cậy để ưu tiên theo dõi ngành.
    Khác với Flow hôm nay: Conviction phạt mạnh các ngành thiếu độ bền/thiếu leader thật.
    """
    flow = _to_num(r.get("score", 0), 0.0)
    strength = _to_num(r.get("sector_strength_score", flow), flow)
    persistence = _to_num(r.get("persistence_score", 0), 0.0)
    leader_score = _leader_score_from_count(r.get("true_leader_count", 0))
    score = 0.30 * flow + 0.40 * strength + 0.20 * persistence + 0.10 * leader_score
    return round(max(0.0, min(100.0, score)), 1)


def _fmt_conviction_row(idx: int, sec: str, r: Dict[str, Any]) -> str:
    score = _to_num(r.get("conviction_score", _sector_conviction_score(r)), 0.0)
    pc = r.get("persistence_count", 0)
    pdn = r.get("persistence_denominator", 0)
    return (
        f"{idx}. {sec}: Conviction {score}/100 | {_sector_conviction_label(score)}\n"
        f"   Flow {r.get('score')}/100 | Strength {r.get('sector_strength_score', '')}/100 | "
        f"Bền vững {pc}/{pdn} | Leader thật {r.get('true_leader_count', 0)}"
    )
...
def evaluate_institutional_flow(
    ...
    # PHASE 18.1 — Sector Conviction: điểm ưu tiên theo dõi ngành đã lọc độ bền và leader thật.
    for _sec, _r in sector_scores.items():
        _conv = _sector_conviction_score(_r)
        _r["conviction_score"] = _conv
        _r["conviction_label"] = _sector_conviction_label(_conv)
    ...
    conviction_ranked = sorted(sector_scores.items(), key=lambda kv: kv[1].get("conviction_score", 0), reverse=True)
    conviction_lines = [_fmt_conviction_row(i, sec, r) for i, (sec, r) in enumerate(conviction_ranked[:7], start=1)]
