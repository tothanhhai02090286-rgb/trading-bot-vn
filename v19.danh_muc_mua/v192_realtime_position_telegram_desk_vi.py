...
def build_alert_message(row: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Sức khỏe: <b>{row.get('Position Health Score', '')}/100</b> - {row.get('Position Health Level', '')}\n"
        f"Đánh giá: <b>{row.get('Position Health Verdict', '')}</b>\n"
        f"Risk Score: <b>{row.get('Position Risk Score', '')}/100</b>\n"
        f"Lý do: {row.get('Position Health Reasons', '')}\n\n"
        f"<b>Vị thế:</b>\n"
