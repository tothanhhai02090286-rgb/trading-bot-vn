...
try:
    from vn_ma_proximity_scanner import evaluate_ma_proximity
    VN_MA_PROXIMITY_ON = os.getenv("VN_MA_PROXIMITY_ON", "1").strip() == "1"
except Exception as e:
    print("WARN VN MA proximity scanner import failed:", repr(e), flush=True)
    VN_MA_PROXIMITY_ON = False
...
MA_PROXIMITY_TOLERANCE_PCT = float(os.getenv("VN_MA_PROXIMITY_TOLERANCE_PCT", "1.0"))
MA_PROXIMITY_TOP_N = int(os.getenv("VN_MA_PROXIMITY_TOP_N", "6"))
MA_PROXIMITY_MAX_SYMBOLS = int(os.getenv("VN_MA_PROXIMITY_MAX_SYMBOLS", "0"))
...
TELEGRAM_SAFE_LIMIT = int(os.getenv("TELEGRAM_SAFE_LIMIT", "3400"))


def split_telegram_message(text: str, limit: int = TELEGRAM_SAFE_LIMIT) -> List[str]:
    """Chia tin Telegram dài thành nhiều phần để tránh lỗi 400: message is too long.

    Telegram giới hạn khoảng 4096 ký tự. Bot dùng HTML nên đặt ngưỡng an toàn thấp hơn.
    Chia theo dòng để hạn chế cắt giữa thẻ HTML.
    """
    if text is None:
        return []
    text = str(text)
    ...
    for line in text.splitlines(True):
        # Nếu một dòng đơn quá dài, cắt thô theo limit. Trường hợp này hiếm.
        if len(line) > limit:
            if current.strip():
...
def enrich_with_institutional_flow(snapshot: pd.DataFrame, alerts: pd.DataFrame, watchlist: pd.DataFrame, positions: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    if not VN_INSTITUTIONAL_FLOW_ON:
        flow = {
            ...
            "position_sector_notes": "", "notes": "", "recommendation_lines": "", "sector_conviction_ranking": "",
        }
    else:
        try:
            ...
            flow = {
                ...
                "top_sectors": "", "weak_sectors": "", "accumulation_sectors": "", "distribution_sectors": "",
                "banking_score": "", "securities_score": "", "real_estate_score": "",
                "position_sector_notes": "", "notes": repr(e), "recommendation_lines": "⚠️ Kiểm tra log Institutional Flow", "sector_conviction_ranking": "",
            }
    ...
    cols = {
        ...
        "Institutional Flow Conviction Ranking": flow.get("sector_conviction_ranking", ""),
    }
...
def enrich_with_market_internals(snapshot: pd.DataFrame, alerts: pd.DataFrame, positions: pd.DataFrame, institutional_flow: Optional[Dict[str, Any]] = None) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    if not VN_MARKET_INTERNALS_ON:
        mi = {
            ...
            "market_liquidity_value_bn": "", "market_liquidity_avg20_bn": "", "market_liquidity_vs20_pct": "",
            "distribution_days_20": "", "distribution_label": "",
            "distribution_recent_dates": "", "distribution_latest_date": "", "distribution_latest_note": "",
    ...
    cols = {
        ...
        "Market Liquidity Value Bn": mi.get("market_liquidity_value_bn", ""),
        "Market Liquidity Avg20 Bn": mi.get("market_liquidity_avg20_bn", ""),
        "Market Liquidity Vs20 Pct": mi.get("market_liquidity_vs20_pct", ""),
        ...
        "Market Internals Distribution Recent Dates": mi.get("distribution_recent_dates", ""),
        "Market Internals Distribution Latest Date": mi.get("distribution_latest_date", ""),
        "Market Internals Distribution Latest Note": mi.get("distribution_latest_note", ""),
        "Market Internals Distribution Latest Value Bn": mi.get("distribution_latest_value_bn", ""),
        "Market Internals Distribution Latest Avg20 Bn": mi.get("distribution_latest_avg20_bn", ""),
        "Market Internals Distribution Latest Vs20 Pct": mi.get("distribution_latest_vs20_pct", ""),
        "Distribution Pressure Score": mi.get("distribution_pressure_score", ""),
        "Distribution Pressure Label": mi.get("distribution_pressure_label", ""),
        "Correction Probability 5D": mi.get("correction_prob_5d", ""),
        "Sideway Probability 5D": mi.get("sideway_prob_5d", ""),
        "Recovery Probability 5D": mi.get("recovery_prob_5d", ""),
        "Correction Probability 10D": mi.get("correction_prob_10d", ""),
        "Sideway Probability 10D": mi.get("sideway_prob_10d", ""),
        "Breakout Probability 10D": mi.get("breakout_prob_10d", ""),
        "Market Probability Forecast Label": mi.get("probability_forecast_label", ""),
        "Market Probability Forecast Recommendation": mi.get("probability_forecast_recommendation", ""),
...
def enrich_with_ma_proximity(snapshot: pd.DataFrame, alerts: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    """PHASE 17 — MÃ GẦN MA20 / MA50 (MA Proximity Scanner).

    Chỉ bổ sung danh sách quan sát các mã gần MA20/MA50 trong biên độ cấu hình,
    không thay đổi action mua/bán của V19.2.
    """
    if not VN_MA_PROXIMITY_ON:
        ma = {
            "status": "OFF", "icon": "", "label_vi": "",
            "tolerance_pct": "", "universe_count": "", "valid_count": "",
            "ma20_count": "", "ma50_count": "",
            "data_last_updated": "", "data_freshness_icon": "", "data_freshness_label": "", "data_age_hours": "",
            "near_ma20_lines": "", "near_ma50_lines": "", "notes": "",
        }
    else:
        try:
            res = evaluate_ma_proximity(
                cache_dir=CACHE_DIR,
                tolerance_pct=MA_PROXIMITY_TOLERANCE_PCT,
                max_symbols=MA_PROXIMITY_MAX_SYMBOLS,
                top_n=MA_PROXIMITY_TOP_N,
            )
            ma = res.to_dict() if hasattr(res, "to_dict") else dict(res)
        except Exception as e:
            ma = {
                "status": "ERROR", "icon": "⚠️", "label_vi": "Không tính được danh sách mã gần MA20/MA50",
                "tolerance_pct": MA_PROXIMITY_TOLERANCE_PCT, "universe_count": "", "valid_count": "",
                "ma20_count": "", "ma50_count": "",
                "data_last_updated": "", "data_freshness_icon": "⚪", "data_freshness_label": "Không rõ", "data_age_hours": "",
                "near_ma20_lines": "", "near_ma50_lines": "", "notes": repr(e),
            }

    cols = {
        "MA Proximity Status": ma.get("status", ""),
        "MA Proximity Icon": ma.get("icon", ""),
        "MA Proximity Label VI": ma.get("label_vi", ""),
        "MA Proximity Tolerance %": ma.get("tolerance_pct", ""),
        "MA Proximity Universe Count": ma.get("universe_count", ""),
        "MA Proximity Valid Count": ma.get("valid_count", ""),
        "MA20 Proximity Count": ma.get("ma20_count", ""),
        "MA50 Proximity Count": ma.get("ma50_count", ""),
        "MA Proximity Last Updated": ma.get("data_last_updated", ""),
        "MA Proximity Freshness Icon": ma.get("data_freshness_icon", ""),
        "MA Proximity Freshness Label": ma.get("data_freshness_label", ""),
        "MA Proximity Age Hours": ma.get("data_age_hours", ""),
        "Near MA20 Lines": ma.get("near_ma20_lines", ""),
        "Near MA50 Lines": ma.get("near_ma50_lines", ""),
        "MA Proximity Notes": ma.get("notes", ""),
    }
    ...
    return _add_cols(snapshot), _add_cols(alerts), ma
...
def build_ma_proximity_block(row: Dict[str, Any]) -> str:
    status = row.get("MA Proximity Status", "")
    if not status or status == "OFF":
        return ""
    return (
        f"<b>🎯 MÃ GẦN MA20 / MA50</b>\n"
        f"<i>(MA Proximity Scanner)</i>\n\n"
        f"Trạng thái: <b>{row.get('MA Proximity Icon', '')} {row.get('MA Proximity Label VI', '')}</b>\n"
        f"Biên độ quét: <b>+/- {row.get('MA Proximity Tolerance %', '')}%</b> | "
        f"Dữ liệu: <b>{row.get('MA Proximity Valid Count', '')}/{row.get('MA Proximity Universe Count', '')}</b> mã\n"
        f"Last Updated: <b>{row.get('MA Proximity Last Updated', '')}</b> | "
        f"{row.get('MA Proximity Freshness Icon', '')} <b>{row.get('MA Proximity Freshness Label', '')}</b>"
        f" | Age: <b>{row.get('MA Proximity Age Hours', '')}h</b>\n\n"
        f"🎯 <b>GẦN MA20</b> — hỗ trợ/ngưỡng ngắn hạn\n"
        f"Số mã: <b>{row.get('MA20 Proximity Count', '')}</b>\n"
        f"{row.get('Near MA20 Lines', '')}\n\n"
        f"🎯 <b>GẦN MA50</b> — hỗ trợ/ngưỡng trung hạn\n"
        f"Số mã: <b>{row.get('MA50 Proximity Count', '')}</b>\n"
        f"{row.get('Near MA50 Lines', '')}\n"
    )



def _pf_float(x: Any, default: float = 0.0) -> float:
    try:
        if x is None:
            return default
        if isinstance(x, str):
...
def _pf_clamp(v: float, lo: float = 0.0, hi: float = 100.0) -> float:
    return max(lo, min(hi, float(v)))


def _portfolio_status(score: float) -> str:
    if score >= 80:
        return '🟢 XÁC SUẤT THÀNH CÔNG CAO'
    if score >= 60:
        return '🟡 TÍCH CỰC THẬN TRỌNG'
    if score >= 40:
        return '🟠 TRUNG TÍNH / PHÒNG THỦ'
    return '🔴 RỦI RO CAO'
...
def _action_penalty(action: str) -> float:
    ...
    if a == 'THOÁT VỊ THẾ':
        return 18.0
    if a in ['CẮT LỖ', 'GIẢM TỶ TRỌNG', 'GIẢM VỊ THẾ', 'KIỂM TRA GIÁ TRƯỚC KHI BÁN']:
        return 10.0
    if 'CHƯA BÁN ĐƯỢC' in a:
        return 7.0
    if a in ['GIỮ', 'GIỮ CÓ KIỂM SOÁT']:
        return -2.0
    return 0.0


def _build_portfolio_probability_block(snapshot: pd.DataFrame) -> str:
    """PHASE 18.3 — DỰ BÁO XÁC SUẤT DANH MỤC.

    Đây là ước lượng thống kê theo dữ liệu hiện tại của chính danh mục:
    Health vị thế, sức mạnh tương đối, ngành, xác suất thị trường, thanh khoản và rủi ro tập trung.
    Không thay đổi action mua/bán.
    """
    try:
        ...
        weights = pd.to_numeric(df.get('Tỷ trọng hiện tại %', 0), errors='coerce').fillna(0.0)
        if float(weights.sum()) <= 0:
            weights = pd.Series([1.0 / max(len(df), 1)] * len(df), index=df.index) * 100.0
        w = weights / max(float(weights.sum()), 1e-9)

        health = pd.to_numeric(df.get('Position Health Score', 50), errors='coerce').fillna(50.0).clip(0, 100)
        weighted_health = float((health * w).sum())

        # Relative strength proxy: dùng P/L của vị thế so với bối cảnh thị trường.
        # Mã tăng/giảm ít trong thị trường xấu thường được điểm tốt hơn.
        pnl = pd.to_numeric(df.get('Lãi/lỗ %', 0), errors='coerce').fillna(0.0)
        rs_score = pnl.apply(lambda x: 75.0 if x >= 1.0 else (60.0 if x >= 0 else (50.0 if x >= -1.0 else (40.0 if x >= -3.0 else 25.0))))
        weighted_rs = float((rs_score * w).sum())

        sector_score = pd.to_numeric(df.get('Sector Score', 50), errors='coerce').fillna(50.0).clip(0, 100)
        weighted_sector = float((sector_score * w).sum())

        first = df.iloc[0].to_dict()
        correction5 = _pf_float(first.get('Correction Probability 5D', 50), 50.0)
        recovery5 = _pf_float(first.get('Recovery Probability 5D', 20), 20.0)
        correction10 = _pf_float(first.get('Correction Probability 10D', correction5), correction5)
        breakout10 = _pf_float(first.get('Breakout Probability 10D', recovery5), recovery5)
        market_score = _pf_clamp(100.0 - correction5 + recovery5 * 0.25, 0, 100)

        liquidity_pct = _pf_float(first.get('Market Internals Participation Pct', 100), 100.0)
        liquidity_score = _pf_clamp(liquidity_pct, 0, 140) / 1.4
        ...
        score = (
            0.30 * weighted_health
            + 0.20 * weighted_rs
            + 0.20 * weighted_sector
            + 0.15 * market_score
        ...
        penalties = 0.0
        positive, negative, contrib = [], [], []
        for _, r in df.iterrows():
            sym = safe_str(r.get('Mã', ''))
            wt = _pf_float(r.get('Tỷ trọng hiện tại %', 0), 0.0)
            h = _pf_float(r.get('Position Health Score', 50), 50.0)
            p = _pf_float(r.get('Lãi/lỗ %', 0), 0.0)
            act = safe_str(r.get('Hành động V19.2', ''))
            trend = safe_str(r.get('Trạng thái xu hướng', ''))
            sec_score = _pf_float(r.get('Sector Score', 50), 50.0)

            pen = _action_penalty(act)
            if h < 30:
                pen += 8.0
            if sec_score < 20:
                pen += 4.0
            penalties += pen * (wt / max(float(weights.sum()), 1e-9))

            c = (h - 50.0) * 0.30 + p * 3.0 - pen * 0.6
            contrib.append((c, sym, h, p, act))

            if h >= 70:
                positive.append(f'✅ {sym} Health {h:.0f}/100 đang nâng đỡ danh mục')
            if 'TRÊN MA20' in trend or 'XU HƯỚNG TỐT' in trend:
                positive.append(f'✅ {sym} giữ được xu hướng {trend}')
            if act == 'GIỮ':
                positive.append(f'✅ {sym} đang ở trạng thái GIỮ')

            if act == 'THOÁT VỊ THẾ':
                negative.append(f'⚠️ {sym} báo THOÁT VỊ THẾ')
            elif 'CHƯA BÁN ĐƯỢC' in act:
                negative.append(f'⚠️ {sym} có rủi ro nhưng chưa bán được theo T+')
            if h < 50:
                negative.append(f'⚠️ {sym} Health thấp ({h:.0f}/100)')
            if sec_score < 25:
                negative.append(f'⚠️ {sym} thuộc nhóm ngành yếu / dòng tiền kém')

        if largest_weight >= 30:
            big_sym = safe_str(df.loc[weights.idxmax()].get('Mã', '')) if len(df) else ''
            negative.append(f'⚠️ Danh mục tập trung {largest_weight:.1f}% vào {big_sym}')

        if correction5 >= 60:
            negative.append(f'⚠️ Thị trường đang có xác suất điều chỉnh cao ({correction5:.1f}% trong 5 phiên)')
        elif recovery5 >= 35:
            positive.append(f'✅ Xác suất hồi phục thị trường đang cải thiện ({recovery5:.1f}%)')

        score = _pf_clamp(score - penalties, 0, 100)
        status = _portfolio_status(score)

        # Quy đổi score sang xác suất danh mục. Đây là mô hình heuristic thống kê, không phải dự báo chắc chắn.
        loss5 = _pf_clamp(35.0 + correction5 * 0.35 + (50.0 - score) * 0.45, 8, 88)
        profit5 = _pf_clamp(18.0 + score * 0.35 + recovery5 * 0.20 - correction5 * 0.15, 5, 75)
        if profit5 + loss5 > 92:
            scale = 92.0 / (profit5 + loss5)
            profit5 *= scale
            loss5 *= scale
        side5 = max(5.0, 100.0 - profit5 - loss5)
        total5 = profit5 + side5 + loss5
        profit5, side5, loss5 = [round(x / total5 * 100.0, 1) for x in (profit5, side5, loss5)]

        loss10 = _pf_clamp(loss5 * 0.85 + correction10 * 0.30 + (50.0 - score) * 0.20, 8, 90)
        profit10 = _pf_clamp(profit5 * 0.70 + breakout10 * 0.25 + score * 0.12, 5, 80)
        if profit10 + loss10 > 92:
            scale = 92.0 / (profit10 + loss10)
            profit10 *= scale
            loss10 *= scale
        side10 = max(5.0, 100.0 - profit10 - loss10)
        total10 = profit10 + side10 + loss10
        profit10, side10, loss10 = [round(x / total10 * 100.0, 1) for x in (profit10, side10, loss10)]

        contrib_sorted = sorted(contrib, key=lambda x: x[0], reverse=True)
        best = contrib_sorted[:3]
        worst = contrib_sorted[-3:][::-1]
        best_lines = []
        for c, sym, h, p, act in best:
            best_lines.append(f'🟢 {sym}: {c:+.1f} điểm | Health {h:.0f} | P/L {p:.2f}%')
        worst_lines = []
        for c, sym, h, p, act in worst:
            worst_lines.append(f'🔴 {sym}: {c:+.1f} điểm | Health {h:.0f} | {act}')

        pos_lines = '\n'.join(list(dict.fromkeys(positive))[:4]) or '✅ Chưa có yếu tố tích cực nổi bật.'
        neg_lines = '\n'.join(list(dict.fromkeys(negative))[:5]) or '⚪ Chưa có rủi ro nổi bật.'

        if score >= 70:
            conclusion = 'Danh mục đang khỏe hơn bối cảnh chung; ưu tiên giữ mã khỏe và không mua đuổi.'
        elif score >= 50:
            conclusion = 'Danh mục trung tính; ưu tiên giữ mã mạnh hơn VNINDEX và hạn chế nâng tỷ trọng.'
        else:
            conclusion = 'Danh mục đang chịu rủi ro cao; ưu tiên giảm rủi ro ở mã yếu khi hàng bán được.'

        return (
            f"📊 <b>DỰ BÁO XÁC SUẤT DANH MỤC</b> <i>(Portfolio Probability Forecast)</i>\n"
            f"Portfolio Score: <b>{score:.1f}/100</b> | <b>{status}</b>\n\n"
            f"5 phiên tới: 🟢 Sinh lời <b>{profit5}%</b> | 🟡 Đi ngang <b>{side5}%</b> | 🔴 Thua lỗ <b>{loss5}%</b>\n"
            f"10 phiên tới: 🟢 Lãi &gt;5% <b>{profit10}%</b> | 🟡 Dao động nhỏ <b>{side10}%</b> | 🔴 Lỗ &gt;5% <b>{loss10}%</b>\n\n"
            f"✅ <b>Yếu tố tích cực</b>\n{pos_lines}\n\n"
            f"⚠️ <b>Yếu tố tiêu cực</b>\n{neg_lines}\n\n"
            f"📊 <b>Đóng góp vào sức khỏe danh mục</b>\n"
            f"{chr(10).join(best_lines)}\n"
            f"{chr(10).join(worst_lines)}\n\n"
            f"🎯 <b>Kết luận danh mục:</b> {conclusion}\n"
            f"Ghi chú: Đây là ước lượng thống kê theo dữ liệu hiện tại của danh mục, không phải dự đoán chắc chắn.\n\n"
        )
    except Exception as e:
        return f"📊 <b>DỰ BÁO XÁC SUẤT DANH MỤC</b>\n⚠️ Không tính được Portfolio Probability Forecast: <code>{repr(e)}</code>\n\n"
...
def build_market_internals_block(row: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"GTGD ngày cuối: <b>{row.get('Market Liquidity Value Bn', '')} tỷ</b> | GTGD TB20: <b>{row.get('Market Liquidity Avg20 Bn', '')} tỷ</b> | Bằng: <b>{row.get('Market Liquidity Vs20 Pct', '')}% TB20</b>\n\n"
        ...
        f"Gần nhất: <b>{row.get('Market Internals Distribution Latest Date', '') or 'Không có'}</b>\n"
        f"Các phiên gần đây: <b>{row.get('Market Internals Distribution Recent Dates', '') or 'Không có'}</b>\n"
        f"{row.get('Market Internals Distribution Latest Note', '')}\n"
        f"GTGD phiên phân phối gần nhất: <b>{row.get('Market Internals Distribution Latest Value Bn', '')} tỷ</b> | GTGD TB20 trước phiên đó: <b>{row.get('Market Internals Distribution Latest Avg20 Bn', '')} tỷ</b>\n\n"
        f"📊 <b>DỰ BÁO XÁC SUẤT THỊ TRƯỜNG</b> <i>(Market Probability Forecast)</i>\n"
        f"Áp lực phân phối: <b>{row.get('Distribution Pressure Score', '')}/100</b> | {row.get('Distribution Pressure Label', '')}\n"
        f"5 phiên tới: 🔴 Điều chỉnh <b>{row.get('Correction Probability 5D', '')}%</b> | 🟡 Đi ngang <b>{row.get('Sideway Probability 5D', '')}%</b> | 🟢 Hồi phục <b>{row.get('Recovery Probability 5D', '')}%</b>\n"
        f"10 phiên tới: 🔴 Giảm >3% <b>{row.get('Correction Probability 10D', '')}%</b> | 🟡 Sideway <b>{row.get('Sideway Probability 10D', '')}%</b> | 🟢 Bứt phá <b>{row.get('Breakout Probability 10D', '')}%</b>\n"
        f"Kết luận xác suất: <b>{row.get('Market Probability Forecast Label', '')}</b>\n"
        f"{row.get('Market Probability Forecast Recommendation', '')}\n"
        f"Ghi chú: Đây là ước lượng thống kê theo dữ liệu hiện tại, không phải dự đoán chắc chắn.\n\n"
...
def build_institutional_flow_block(row: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"🔥 <b>SECTOR CONVICTION RANKING</b>\n"
        f"{row.get('Institutional Flow Conviction Ranking', '')}\n\n"
...
def compact_position_line(row: Dict[str, Any]) -> str:
    """Một dòng tóm tắt vị thế cho Telegram.

    Mục tiêu: nhìn nhanh trên iPhone, không lặp lại các block thị trường lớn.
    """
    symbol = row.get("Mã", "")
    action = row.get("Hành động V19.2", "")
    emoji = emoji_for_action(action)
    pnl = row.get("Lãi/lỗ %", "")
    weight = row.get("Tỷ trọng hiện tại %", "")
    health = row.get("Position Health Score", "")
    health_icon = row.get("Position Health Icon", "")
    trend = row.get("Trạng thái xu hướng", "")
    sellable = row.get("Bán được chưa?", "")
    return (
        f"{emoji} <b>{symbol}</b>: <b>{action}</b> | "
        f"P/L <b>{pnl}%</b> | Tỷ trọng <b>{weight}%</b> | "
        f"Health <b>{health}</b>{health_icon} | {trend} | Bán: <b>{sellable}</b>"
    )


def build_positions_summary_block(snapshot: pd.DataFrame) -> str:
    """PHASE 17.1 — tóm tắt toàn bộ danh mục một lần trong startup."""
    if snapshot is None or snapshot.empty:
        return ""
    lines = ["<b>📊 TÓM TẮT VỊ THẾ</b>", "<i>(Smart Telegram Summary)</i>", ""]
    for _, r in snapshot.iterrows():
        lines.append(compact_position_line(r.to_dict()))
    lines.append("")
    # Chỉ nhắc nhanh các mã cần chú ý, không bung chi tiết tại startup.
    try:
        risky = snapshot[~snapshot["Hành động V19.2"].isin(["GIỮ", "THEO DÕI VỊ THẾ"])]
        if not risky.empty:
            symbols = ", ".join(risky["Mã"].astype(str).tolist())
            lines.append(f"⚠️ Mã cần đọc chi tiết: <b>{symbols}</b>")
            lines.append("")
    except Exception:
        pass
    return "\n".join(lines)
...
def build_alert_message(row: Dict[str, Any]) -> str:
    """Tin chi tiết cho từng mã — bản gọn, không lặp lại các block thị trường.

    Các block lớn như Institutional Flow, Market Internals, Portfolio Intelligence,
    Mini Market, Leader Rotation đã nằm trong STARTED summary. Tin từng mã chỉ giữ
    phần cần hành động: kết luận, sức khỏe, giá, stop, T+, safety và sector của mã.
    """
    symbol, action = row.get("Mã", ""), row.get("Hành động V19.2", "")
    emoji = emoji_for_action(action)
    return (
        f"{emoji} <b>[V19.2 POSITION]</b> <b>{symbol}</b>\n\n"
        f"<b>KẾT LUẬN:</b> <b>{action}</b>\n"
        f"<b>DỄ HIỂU:</b> {row.get('Kết luận dễ hiểu', '')}\n"
        f"<b>LÝ DO:</b> {row.get('Lý do chính', '')}\n\n"

        f"<b>📌 TÓM TẮT NHANH</b>\n"
        f"Lãi/lỗ: <b>{row.get('Lãi/lỗ %', '')}%</b> | Tỷ trọng: <b>{row.get('Tỷ trọng hiện tại %', '')}%</b>\n"
        f"Health: <b>{row.get('Position Health Score', '')}/100</b> {row.get('Position Health Icon', '')} {row.get('Position Health Level', '')}\n"
        f"Rủi ro: <b>{row.get('Position Risk Score', '')}/100</b> {row.get('Position Risk Icon', '')} {row.get('Position Risk Level', '')}\n"
        f"Trend: <b>{row.get('Trạng thái xu hướng', '')}</b> | Stop: <b>{row.get('Stop đề xuất', '')}</b> ({row.get('Loại stop chính', '')})\n\n"

        f"<b>🛡️ TRẠNG THÁI VỊ THẾ</b> <i>(Position State)</i>\n"
        ...
        f"<b>❤️ SỨC KHỎE VỊ THẾ</b> <i>(Position Health)</i>\n"
        f"Nguyên nhân:\n{row.get('Position Health Reasons Bullets', '') or row.get('Position Health Reasons', '')}\n"
        f"Khuyến nghị: {row.get('Position Health Recommendation', '') or action}\n\n"

        f"<b>📦 TỒN KHO T+ VIỆT NAM</b> <i>(Settlement Inventory)</i>\n"
        f"T0: <b>{row.get('KL_T0', 0)}</b> | T1: <b>{row.get('KL_T1', 0)}</b> | T2: <b>{row.get('KL_T2', 0)}</b> | Bán được: <b>{row.get('KL_Bán_Được', 0)}</b>\n"
        f"Ngày dự kiến bán được: <b>{row.get('Ngày dự kiến bán được', '')}</b>\n"
        f"Ghi chú: {row.get('Ghi chú T+2.5', '')}\n\n"

        f"<b>💵 GIÁ & STOP</b>\n"
        f"Giá vốn: <b>{row.get('Giá vốn', '')}</b> | Giá hiện tại: <b>{row.get('Giá hiện tại', '')}</b>\n"
        f"Nguồn giá: <b>{row.get('Nguồn giá', '')}</b> | Realtime OK: <b>{row.get('Realtime OK', '')}</b>\n"
        f"Thời gian giá: <b>{row.get('Thời gian giá', '')}</b>\n"
        f"Ghi chú giá: {row.get('Ghi chú giá', '')}\n\n"

        f"<b>🌊 DÒNG TIỀN NGÀNH CỦA MÃ</b> <i>(Sector Money Flow)</i>\n"
        f"Ngành: <b>{row.get('Sector', '')}</b> | Flow: <b>{row.get('Sector Flow', '')}</b> | Score: <b>{row.get('Sector Score', '')}</b>\n"
        f"Leader: <b>{row.get('Sector Leaders', '')}</b>\n"
        f"Ghi chú: {row.get('Sector Note', '')}\n\n"

        f"<b>✅ AN TOÀN GIAO DỊCH</b> <i>(VN Trade Safety)</i>\n"
        ...
        f"<b>Upstream Risk:</b> Final <b>{row.get('Quyết định cuối', '')}</b> | Mode <b>{row.get('Chế độ đánh', '')}</b>\n"
        f"Time: {now_str()}"
    )
...
def build_startup_message(snapshot: pd.DataFrame, alerts: pd.DataFrame) -> str:
    ...
    ma_proximity_startup_block = ""
    positions_summary_block = ""
    try:
        if not snapshot.empty:
            ...
            ma_proximity_startup_block = build_ma_proximity_block(first_row)
            portfolio_probability_startup_block = _build_portfolio_probability_block(snapshot)
            positions_summary_block = build_positions_summary_block(snapshot)
    except Exception as e:
    ...
    return (
        ...
        f"Portfolio Probability Forecast: <b>ON</b>\n"
        f"Time: {now_str()}\n\n"
        f"{institutional_flow_startup_block}"
        f"{market_internals_startup_block}"
        f"{portfolio_probability_startup_block}"
        f"{ma_proximity_startup_block}"
        f"{positions_summary_block}"
    )
...
def scan_once(state: Dict[str, Any], startup: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    snapshot, alerts, ma_proximity = enrich_with_ma_proximity(snapshot, alerts)
