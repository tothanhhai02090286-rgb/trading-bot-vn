...
def enrich_with_mini_market_regime(snapshot: pd.DataFrame, alerts: pd.DataFrame, watchlist: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    if not VN_MINI_MARKET_REGIME_ON:
        regime = {
            ...
            "valid_count": 0, "pct_up": "", "pct_above_ma20": "", "pct_above_ma50": "",
...
def build_mini_market_block(row: Dict[str, Any]) -> str:
    ...
    return (
        f"<b>🌏 Mini Market Regime - {universe} mã</b>\n\n"
        f"Trạng thái: <b>{icon} {regime}</b>\n"
        f"Điểm thị trường: <b>{score}/100</b>\n\n"
        f"Breadth:\n"
        f"• {row.get('Mini Market Pct Up', '')}% mã tăng\n"
        f"• {row.get('Mini Market Pct Above MA20', '')}% trên MA20\n"
        f"• {row.get('Mini Market Pct Above MA50', '')}% trên MA50\n\n"
        f"Leadership:\n"
        f"• {row.get('Mini Market Pct Healthy', '')}% mã khỏe\n\n"
        f"Risk Pressure:\n"
        f"• {row.get('Mini Market Pct Weak', '')}% mã yếu\n\n"
        f"Money Flow:\n"
        f"• <b>{row.get('Mini Market Money Flow', '')}</b> | Score: <b>{row.get('Mini Market Money Flow Score', '')}</b>\n\n"
        f"Khuyến nghị:\n{rec_lines}\n"
        f"{notes_line}\n"
    )
