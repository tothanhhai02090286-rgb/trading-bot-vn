...
def enrich_with_leader_rotation(snapshot: pd.DataFrame, alerts: pd.DataFrame, watchlist: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    if not VN_LEADER_ROTATION_ON:
        ...
        try:
            rotation = evaluate_leader_rotation(
                watchlist, cache_dir=CACHE_DIR, max_symbols=LEADER_ROTATION_MAX_SYMBOLS
            ).to_dict()
        except Exception as e:
            rotation = {
...
def build_leader_rotation_block(row: Dict[str, Any]) -> str:
    ...
    score = row.get("Leader Rotation Score", "")
    notes = row.get("Leader Rotation Notes", "")
    notes_line = f"Ghi chú: {notes}\n" if notes else ""
    return (
        f"<b>🔥 Leader Rotation Lite</b>\n\n"
        f"Trạng thái: <b>{icon} {status}</b> | Score: <b>{score}/100</b>\n"
...
def scan_once(state: Dict[str, Any], startup: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    snapshot, alerts, leader_rotation = enrich_with_leader_rotation(snapshot, alerts, watchlist)
    write_outputs(snapshot, alerts)
