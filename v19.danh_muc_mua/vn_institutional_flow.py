...
def evaluate_institutional_flow(
    ...
    return InstitutionalFlowResult(
        ...
        notes=f"Score là dòng tiền tương đối theo giá + breadth + volume, không phải kết luận tiền lớn mua/bán thật. Tính từ {len(df)} mã hợp lệ / {len(symbols)} mã cache; {len(sector_scores)} ngành đủ dữ liệu.",
