...
def _sector_symbol_leaders(g: pd.DataFrame, max_items: int = 3) -> Tuple[str, str]:
    """Trả về mã mạnh nhất/yếu nhất trong từng ngành dựa trên 5D return.

    Đây là ngữ cảnh quan sát, không phải khuyến nghị mua/bán.
    """
    ...
    if tmp.empty:
        return "-", "-"
    top = [(str(r["Mã"]).upper(), float(r["ret5_pct"])) for _, r in tmp.sort_values("ret5_pct", ascending=False).head(max_items).iterrows()]
    weak = [(str(r["Mã"]).upper(), float(r["ret5_pct"])) for _, r in tmp.sort_values("ret5_pct", ascending=True).head(max_items).iterrows()]
    return _fmt_symbol_list(top, max_items), _fmt_symbol_list(weak, max_items)
...
def _fmt_sector_row(sec: str, r: Dict[str, Any]) -> str:
    # Giữ format gọn để không làm Telegram quá dài.
    return (
        f"- {sec}: {r.get('score')}/100 | {r.get('label')} | {r.get('flow_status', '')} | 5D {r.get('avg_ret5_pct')}% | Vol {r.get('avg_volume_ratio20')}x\n"
        f"  Mã mạnh: {r.get('leader_symbols', '-')} | Mã yếu: {r.get('weak_symbols', '-')}"
    )


def _fmt_position_flow_note(sym: str, sec: str, ss: Dict[str, Any]) -> str:
    return f"- {sym}: {sec} | {ss.get('score')}/100 | {ss.get('flow_status', ss.get('label', ''))}"
