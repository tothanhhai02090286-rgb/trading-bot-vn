...
def main():
    ...
    # NEW: map proven patterns back to today's tickers
    try:
        top_codes_pattern_view = build_top_codes_by_proven_pattern(
            combined,
            top_patterns_view,
            limit=20
        )
    except Exception as e:
        print("WARN: top codes by proven pattern error:", repr(e))
        top_codes_pattern_view = pd.DataFrame()
    ...
    top_codes_pattern_html = top_codes_pattern_view.to_html(index=False, escape=True)
...
<h3>TOP CODES BY PROVEN PATTERN</h3>
{top_codes_pattern_html}
