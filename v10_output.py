...
# ============================================================
# V10 CONFIDENCE SCORE - DIEM TIN CAY TIN HIEU
# Display-only layer. Does NOT change core trading logic.
# ============================================================

def calc_signal_confidence_score(row, mode="T5"):
    score = 0

    oos = safe_float(row.get("OOS%"), np.nan)
    oosn = safe_float(row.get("OOSN"), 0)
    avg_used = safe_float(row.get("Avg Used"), np.nan)
    rs20 = safe_float(row.get("RS20"), np.nan)
    ai = safe_float(row.get("AI"), safe_float(row.get("AI Confidence"), np.nan))
    base_score = safe_float(row.get("Score"), np.nan)
    risk = str(row.get("Risk Status", "")).upper()
...
def confidence_reason(row, mode="T5"):
    reasons = []
    ...
    if not pd.isna(oos):
        if oos >= 80:
            reasons.append("OOS tot")
        elif oos >= 70:
            reasons.append("OOS dat")
    ...
    if oosn >= 10:
        ...
        reasons.append("mau toi thieu")

    if not pd.isna(avg_used):
        if mode == "T2":
            if avg_used > 1:
                reasons.append("Avg+2D manh")
            elif avg_used > 0.5:
                reasons.append("Avg+2D dat")
            elif avg_used <= 0:
                reasons.append("Avg+2D yeu")
        else:
            if avg_used > 2:
                reasons.append("Avg+5D manh")
            elif avg_used > 1:
                reasons.append("Avg+5D dat")
            elif avg_used <= 0:
                reasons.append("Avg+5D yeu")

    if not pd.isna(rs20):
        if rs20 >= 5:
            reasons.append("RS20 manh")
        elif rs20 >= 0:
            reasons.append("RS20 on")
        elif rs20 < -5:
            reasons.append("RS20 yeu")
...
def add_confidence_columns(view, mode="T5"):
    if view is None or view.empty:
        return view

    out = view.copy()
    out["Diem Tin Cay"] = out.apply(lambda r: calc_signal_confidence_score(r, mode=mode), axis=1)
    out["Xep Hang Tin Cay"] = out["Diem Tin Cay"].apply(confidence_grade)
    out["Ly Do Tin Cay"] = out.apply(lambda r: confidence_reason(r, mode=mode), axis=1)

    preferred_front = [
        "Trade Mode", "Match Level", "Date", "Code", "Price", "Rec",
        "Diem Tin Cay", "Xep Hang Tin Cay", "Ly Do Tin Cay"
    ]
    existing_front = [c for c in preferred_front if c in out.columns]
    other_cols = [c for c in out.columns if c not in existing_front]
    out = out[existing_front + other_cols]

    sort_cols = [c for c in ["Diem Tin Cay", "Avg Used", "OOS%", "OOSN", "AI", "Score"] if c in out.columns]
    if sort_cols:
...
_old_build_top_codes_by_proven_pattern_stable = build_top_codes_by_proven_pattern_stable

def build_top_codes_by_proven_pattern_stable(signal_df, pattern_df, mode="T5", limit=5):
    view = _old_build_top_codes_by_proven_pattern_stable(signal_df, pattern_df, mode=mode, limit=limit)
    view = add_confidence_columns(view, mode=mode)
    if view is None or view.empty:
        return view
...
# ============================================================
# PHAN TICH LY DO BI LOAI - FAIL ANALYSIS DASHBOARD
# Display-only layer. Does NOT change core trading logic.
#
# Purpose:
# - Show why codes are filtered out or weak.
# - Vietnamese labels for user-friendly dashboard.
# - Works with current combined/all signal table.
# ============================================================

def _fa_safe_float(x, default=np.nan):
    try:
...
def _fa_strategy(row):
    for c in ["Chien luoc", "Strategy", "Chiến lược"]:
        if c in row.index:
...
def _fa_date(row):
    for c in ["Ngay", "Date", "Ngày"]:
        if c in row.index:
            return str(row.get(c, ""))
    return ""


def detect_fail_reasons_vi(row):
    """
    Return list of Vietnamese fail/weak reasons for a current signal row.
    This is diagnostic only, not trading logic.
    """
    reasons = []

    strategy = _fa_strategy(row).upper()
    action = _fa_norm_text(row.get("Action", row.get("Final Action", "")))
    risk_status = _fa_norm_text(row.get("Risk Status", ""))

    score = _fa_safe_float(row.get("Score"), np.nan)
    ai = _fa_safe_float(row.get("AI Confidence", row.get("AI")), np.nan)
    rsi = _fa_safe_float(row.get("RSI"), np.nan)
    rs20 = _fa_safe_float(row.get("RS20"), np.nan)
    vol = _fa_safe_float(row.get("Volume Ratio"), np.nan)
    atr = _fa_safe_float(row.get("ATR %"), np.nan)
    adx = _fa_safe_float(row.get("ADX"), np.nan)
    dist_ma20 = _fa_safe_float(row.get("Dist MA20 %"), np.nan)
    ma5 = _fa_safe_float(row.get("MA5"), np.nan)
    ma20 = _fa_safe_float(row.get("MA20"), np.nan)
    close = _fa_safe_float(row.get("Close"), np.nan)
    ret5 = _fa_safe_float(row.get("Ret5 %"), np.nan)
    ret10 = _fa_safe_float(row.get("Ret10 %"), np.nan)

    # Hard risk
    if risk_status == "FAIL":
        reasons.append("Risk FAIL")

    if "SKIP" in action or "BO QUA" in action or "BỎ QUA" in action:
        reasons.append("Action BO QUA/SKIP")

    # Score / AI
    if not pd.isna(score) and score < 70:
        reasons.append("Score thap")
    if not pd.isna(ai) and ai < 65:
        reasons.append("AI thap")

    # RSI diagnostic
    if not pd.isna(rsi):
        if rsi >= 80:
            reasons.append("RSI qua nong")
        elif rsi < 35:
            reasons.append("RSI qua yeu")
        elif "MOMENTUM" in strategy and rsi < 55:
            reasons.append("RSI chua du momentum")
        elif "BOTTOM" in strategy and rsi > 55:
            reasons.append("RSI khong con vung bat day")

    # RS20
    if not pd.isna(rs20):
        if rs20 < -8:
            reasons.append("RS20 yeu hon thi truong manh")
        elif rs20 < 0:
            reasons.append("RS20 hoi yeu")

    # Volume
    if not pd.isna(vol):
        if vol < 0.7:
            reasons.append("Thanh khoan/Volume yeu")
        elif "MOMENTUM" in strategy and vol < 1.0:
            reasons.append("Volume chua xac nhan momentum")

    # ATR
    if not pd.isna(atr):
        if atr > 10:
            reasons.append("ATR qua cao - bien dong lon")
        elif atr < 1:
            reasons.append("ATR qua thap - thieu bien dong")

    # ADX
    if not pd.isna(adx):
        if "MOMENTUM" in strategy and adx < 18:
            reasons.append("ADX yeu - xu huong chua ro")

    # MA / trend
    if not pd.isna(ma5) and not pd.isna(ma20):
        if "MOMENTUM" in strategy and ma5 <= ma20:
            reasons.append("MA5 chua vuot MA20")
    if not pd.isna(close) and not pd.isna(ma20):
        if "MOMENTUM" in strategy and close < ma20:
            reasons.append("Gia duoi MA20")
        if "BOTTOM" in strategy and close > ma20 and not pd.isna(dist_ma20) and dist_ma20 > 8:
            reasons.append("Gia xa MA20 - khong con vung day")

    # Momentum returns
    if "MOMENTUM" in strategy:
        if not pd.isna(ret5) and ret5 <= 0:
            reasons.append("Ret5 chua duong")
        if not pd.isna(ret10) and ret10 <= 0:
            reasons.append("Ret10 chua duong")

    # If no clear fail reason, mark pass/ok
    if not reasons:
        if risk_status == "PASS":
            reasons.append("Dat dieu kien chinh")
        else:
            reasons.append("Can theo doi them")

    return reasons


def build_fail_analysis_summary(df):
    """
    Summary table: reason -> count and percentage.
    """
    if df is None or df.empty:
        return pd.DataFrame([{
            "Ly do": "Khong co du lieu",
            "So lan": 0,
            "Ty le %": 0
        }])
    ...
    for _, r in df.iterrows():
        reasons = detect_fail_reasons_vi(r)
        code = _fa_code(r)
        date = _fa_date(r)
    ...
    if not rows:
        return pd.DataFrame([{
            "Ly do": "Khong phat hien ly do",
            "So lan": 0,
            "Ty le %": 0
        }])

    raw = pd.DataFrame(rows)
    summary = raw.groupby("Ly do").size().reset_index(name="So lan")
    summary["Ty le %"] = (summary["So lan"] / max(total, 1) * 100).round(1)
    summary = summary.sort_values(["So lan", "Ty le %"], ascending=False).reset_index(drop=True)
    return summary


def build_fail_analysis_by_code(df, limit=30):
    """
    Code-level table: code -> reasons.
    """
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            "Ngay": _fa_date(r),
            "Ma": _fa_code(r),
            "Hanh dong": r.get("Action", r.get("Final Action", "")),
            "Chien luoc": _fa_strategy(r),
            "Score": r.get("Score", ""),
            "AI": r.get("AI Confidence", r.get("AI", "")),
            "Risk": r.get("Risk Status", ""),
            "RSI": r.get("RSI", ""),
            "RS20": r.get("RS20", ""),
            "Volume": r.get("Volume Ratio", ""),
            "ATR %": r.get("ATR %", ""),
            "Ly do chinh": "; ".join(reasons[:4])
        })

    out = pd.DataFrame(rows)
    for c in ["Score", "AI", "RSI", "RS20", "Volume", "ATR %"]:
        if c in out.columns:
            out[c] = pd.to_numeric(out[c], errors="coerce")

    # Put weaker / skipped / risk fail first for diagnosis
    if not out.empty:
        out["RiskRank"] = out["Risk"].astype(str).str.upper().apply(lambda x: 0 if x == "FAIL" else 1)
        out = out.sort_values(["RiskRank", "Score", "AI"], ascending=[True, True, True])
        out = out.drop(columns=["RiskRank"], errors="ignore")

    return out.head(limit).replace({np.nan: ""})


def build_fail_analysis_by_strategy(df):
    """
    Strategy-level fail summary.
    """
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    out = pd.DataFrame(rows)
    summary = out.groupby(["Chien luoc", "Ly do"]).size().reset_index(name="So lan")
    summary = summary.sort_values(["Chien luoc", "So lan"], ascending=[True, False]).reset_index(drop=True)
    return summary
