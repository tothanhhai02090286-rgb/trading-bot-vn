...
# ============================================================
# FINAL PRO MAX V2: Core Pattern Matching for T+2 / T+5
# - Fix empty TOP CODES issue caused by exact full-pattern mismatch
# - Match by Pattern Core instead:
#   Regime | Strategy | RSI bucket
# ============================================================

def extract_core_pattern(pattern_value):
    """
    Convert full Pattern Key into a looser core pattern.

    Example:
    UPTREND|MOMENTUM|WAIT|RSI_MID_HIGH|RS_STRONG|...
    -> UPTREND|MOMENTUM|RSI_MID_HIGH

    Why:
    Full pattern is too strict, today's action/vol/atr bucket can differ.
    Core pattern keeps the trading context stable enough to map patterns to codes.
    """
    try:
        parts = [p for p in str(pattern_value).split("|") if p not in ["", "nan", "None"]]
        if len(parts) >= 4:
            # parts[0] = regime, parts[1] = strategy, parts[3] = RSI bucket
...
def _pm_prepare_pattern_table(pattern_df):
    ...
    pat["Pattern Core"] = pat[pattern_col].apply(extract_core_pattern)
...
def _pm_select_patterns(pattern_df, mode="T2"):
    ...
    # VN practical threshold:
    # Tier 1: OOS >= 80 with OOSN >= 3
    # Tier 2: OOS >= 70 with OOSN >= 5
    # Plus positive avg return for the selected horizon.
    mask_strength = (
        ((pat[oos_pct_col] >= 80) & (pat[oos_n_col] >= 3)) |
        ((pat[oos_pct_col] >= 70) & (pat[oos_n_col] >= 5))
    )
    ...
    keep = [pattern_col, "Pattern Core", oos_pct_col, oos_n_col, "Trade Mode", "Avg Type", "Avg Used"]
    for c in ["Avg+2D", "Avg+5D", "Avg+10D", "Trust", "Note", "Source"]:
        if c in strong.columns and c not in keep:
            keep.append(c)

    strong = strong[keep].drop_duplicates(subset=["Pattern Core", "Trade Mode"], keep="first")
    strong = strong.sort_values([oos_pct_col, oos_n_col, "Avg Used"], ascending=False)

    return strong, pattern_col, oos_pct_col, oos_n_col


def _pm_prepare_signal_table(signal_df, current_regime=""):
    sig = normalize_output_columns(signal_df.copy()) if signal_df is not None else pd.DataFrame()
    if sig.empty:
        return sig

    # Ensure Pattern Key exists or create it using current available make_pattern_key
    try:
        if "Pattern Key" not in sig.columns:
            sig["Pattern Key"] = sig.apply(lambda r: make_pattern_key(r, current_regime), axis=1)
        else:
            missing = sig["Pattern Key"].isna() | (sig["Pattern Key"].astype(str).str.strip() == "")
            if missing.any():
                sig.loc[missing, "Pattern Key"] = sig.loc[missing].apply(lambda r: make_pattern_key(r, current_regime), axis=1)
    except Exception:
        # Fallback: build approximate pattern if make_pattern_key unavailable
        if "Pattern Key" not in sig.columns:
            sig["Pattern Key"] = ""

    sig["Pattern Core"] = sig["Pattern Key"].apply(extract_core_pattern)
    return sig
...
def build_top_codes_by_proven_pattern(signal_df, pattern_df, mode="ALL", limit=20):
    """
    FINAL PRO MAX V2:
    Map proven patterns back to today's tickers using Pattern Core matching.

    mode:
    - T2: requires Avg+2D > 0.5
    - T5: requires Avg+5D > 1.0
    - ALL: combine both
    """
    ...
    if sig.empty or "Pattern Core" not in sig.columns:
        return pd.DataFrame()
    ...
    # Rename to phone-friendly names
    rename = {
        "Ngay": "Date",
        "Ma": "Code",
        "Close": "Price",
    ...
    sort_cols = [c for c in ["Trade Mode", "OOS%", "OOSN", "Avg Used", "AI", "Score"] if c in merged.columns]
    if sort_cols:
        ascending = [True] + [False] * (len(sort_cols) - 1) if sort_cols[0] == "Trade Mode" else [False] * len(sort_cols)
    ...
    preferred = [
        ...
        "Trust", "Note", "Score", "AI", "AI Grade",
        "Strategy", "Chien luoc", "RSI", "RS20", "Volume Ratio", "ATR %",
        "Buy Zone", "Stop Loss", "Risk Status", "Why", "Pattern Core", "Pattern Key"
    ]
...
def build_pattern_to_codes_map(signal_df, pattern_df, mode="ALL", limit=20):
    """
    FINAL PRO MAX V2:
    Group tickers by Pattern Core.
    Shows: core pattern -> today's tickers matched.
    """
    ...
    if "Pattern Core" not in codes.columns or "Code" not in codes.columns:
        return pd.DataFrame()

    rows = []
    for core, g in codes.groupby("Pattern Core"):
        g = g.copy()
        best = g.iloc[0]
