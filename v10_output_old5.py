...
# ============================================================
# FINAL PRO MAX V3: Auto fallback matching
# - Level 1: Pattern Core 3 = Regime | Strategy | RSI bucket
# - Level 2: Core 2 = Regime | Strategy
# - Level 3: Core 1 = Regime only
# Goal: avoid empty T+2 / T+5 sections while still ranking by proven pattern stats.
# ============================================================

def extract_pattern_parts(pattern_value):
    try:
        parts = [p for p in str(pattern_value).split("|") if p not in ["", "nan", "None"]]
        return parts
    except Exception:
        return []


def extract_core_pattern(pattern_value):
    parts = extract_pattern_parts(pattern_value)
    if len(parts) >= 4:
...
def extract_core2_pattern(pattern_value):
    parts = extract_pattern_parts(pattern_value)
    if len(parts) >= 2:
        return "|".join(parts[:2])
    return str(pattern_value)
...
def extract_core1_pattern(pattern_value):
    parts = extract_pattern_parts(pattern_value)
    if len(parts) >= 1:
        return parts[0]
    return str(pattern_value)
...
def _pm_prepare_pattern_table(pattern_df):
    ...
    pat["Pattern Full"] = pat[pattern_col].astype(str)
    pat["Pattern Core3"] = pat[pattern_col].apply(extract_core_pattern)
    pat["Pattern Core2"] = pat[pattern_col].apply(extract_core2_pattern)
    pat["Pattern Core1"] = pat[pattern_col].apply(extract_core1_pattern)
...
def _pm_select_patterns(pattern_df, mode="T2"):
    ...
    strong["Trade Mode"] = mode
    strong["Avg Type"] = avg_col
    strong["Avg Used"] = pd.to_numeric(strong[avg_col], errors="coerce")

    keep = [
        pattern_col, "Pattern Full", "Pattern Core3", "Pattern Core2", "Pattern Core1",
        oos_pct_col, oos_n_col, "Trade Mode", "Avg Type", "Avg Used"
    ]
    for c in ["Avg+2D", "Avg+5D", "Avg+10D", "Trust", "Note", "Source"]:
        if c in strong.columns and c not in keep:
            keep.append(c)

    strong = strong[keep].copy()
    strong = strong.sort_values([oos_pct_col, oos_n_col, "Avg Used"], ascending=False)

    return strong, pattern_col, oos_pct_col, oos_n_col


def _pm_prepare_signal_table(signal_df):
    sig = normalize_output_columns(signal_df.copy()) if signal_df is not None else pd.DataFrame()
    if sig.empty:
        return sig

    try:
        if "Pattern Key" not in sig.columns:
            sig["Pattern Key"] = sig.apply(lambda r: make_pattern_key(r, ""), axis=1)
        else:
            missing = sig["Pattern Key"].isna() | (sig["Pattern Key"].astype(str).str.strip() == "")
            if missing.any():
                sig.loc[missing, "Pattern Key"] = sig.loc[missing].apply(lambda r: make_pattern_key(r, ""), axis=1)
    except Exception:
        if "Pattern Key" not in sig.columns:
            sig["Pattern Key"] = ""

    sig["Signal Pattern Full"] = sig["Pattern Key"].astype(str)
    sig["Pattern Core3"] = sig["Pattern Key"].apply(extract_core_pattern)
    sig["Pattern Core2"] = sig["Pattern Key"].apply(extract_core2_pattern)
    sig["Pattern Core1"] = sig["Pattern Key"].apply(extract_core1_pattern)

    return sig


def _pm_merge_with_fallback(sig, strong):
    """
    Try match from strict to loose:
    1) Core3: Regime + Strategy + RSI
    2) Core2: Regime + Strategy
    3) Core1: Regime
    """
    if sig.empty or strong.empty:
        return pd.DataFrame()
    ...
    for col, level_name in merge_levels:
        if col not in sig.columns or col not in strong.columns:
            continue
    ...
    merged = safe_concat_frames(all_matches)

    # Prefer stricter match if same code appears multiple times
    rank_map = {"CORE3": 1, "CORE2": 2, "CORE1": 3}
    merged["Match Rank"] = merged["Match Level"].map(rank_map).fillna(9)

    code_col = "Ma" if "Ma" in merged.columns else "Code" if "Code" in merged.columns else None
    if code_col:
        sort_cols = ["Match Rank"]
        for c in ["OOS%", "OOS Win Probability", "OOS Win Rate", "OOSN", "OOS N", "OOS Samples", "Avg Used", "AI Confidence", "Score"]:
            if c in merged.columns:
                sort_cols.append(c)

        ascending = [True] + [False] * (len(sort_cols) - 1)
        merged = merged.sort_values(sort_cols, ascending=ascending)
        merged = merged.drop_duplicates(subset=[code_col, "Trade Mode"], keep="first")
...
def build_top_codes_by_proven_pattern(signal_df, pattern_df, mode="ALL", limit=20):
    """
    FINAL PRO MAX V3:
    Map proven patterns to today's tickers using auto fallback matching.
    """
    ...
    sig = _pm_prepare_signal_table(signal_df)
    if sig.empty:
        return pd.DataFrame()
    ...
    merged = _pm_merge_with_fallback(sig, strong)
    ...
    # dynamic OOS columns
    if oos_pct_col and oos_pct_col in merged.columns:
        rename[oos_pct_col] = "OOS%"
    if oos_n_col and oos_n_col in merged.columns:
        rename[oos_n_col] = "OOSN"
    ...
    sort_cols = [c for c in ["Match Rank", "OOS%", "OOSN", "Avg Used", "AI", "Score"] if c in merged.columns]
    if sort_cols:
        ascending = [True] + [False] * (len(sort_cols) - 1)
        merged = merged.sort_values(sort_cols, ascending=ascending)
    ...
    preferred = [
        ...
        "Trust", "Note", "Score", "AI", "AI Grade",
        "Strategy", "Chien luoc", "RSI", "RS20", "Volume Ratio", "ATR %",
        "Buy Zone", "Stop Loss", "Risk Status", "Why",
        "Pattern Core3", "Pattern Core2", "Pattern Core1"
    ]
...
def build_pattern_to_codes_map(signal_df, pattern_df, mode="ALL", limit=20):
    ...
    group_col = "Pattern Core2" if "Pattern Core2" in codes.columns else "Pattern Core1" if "Pattern Core1" in codes.columns else None
    if group_col is None or "Code" not in codes.columns:
        return pd.DataFrame()

    rows = []
    for core, g in codes.groupby(group_col):
        g = g.copy()
        best = g.iloc[0]
        top_codes = ", ".join(g["Code"].astype(str).head(10).tolist())

        rows.append({
            "Trade Mode": best.get("Trade Mode", mode),
            "Match Level": best.get("Match Level", ""),
            "Pattern Group": core,
            "OOS%": best.get("OOS%", ""),
            "OOSN": best.get("OOSN", ""),
            "Avg Used": best.get("Avg Used", ""),
    ...
    if not out.empty:
        for c in ["OOS%", "OOSN", "Avg Used", "Num Codes"]:
            out[c] = pd.to_numeric(out[c], errors="coerce")
        out = out.sort_values(["Match Level", "OOS%", "OOSN", "Avg Used", "Num Codes"], ascending=[True, False, False, False, False])
