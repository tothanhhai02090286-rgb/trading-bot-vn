...
def _ui_top_sort(df):
    ...
    sort_cols = []
    for c in [
        "Score", "AI", "AI Confidence", "Điểm lịch sử", "Diem lich su",
        "Mức khớp mẫu %", "Muc khop mau %", "Tỷ lệ thắng", "Ty le thang",
        "Lợi T+5 %", "Loi T+5 %", "Lợi T+2 %", "Loi T+2 %"
    ]:
        col = _ui_find_col(out, [c])
...
<p class="full-note">Lấy từ bảng V13.4, sắp xếp theo độ tin cậy/AI/Score cao nhất.</p>
{v134_green_top_html}
</div>

<div class="top-card top-red">
<h3>TOP ĐỎ - RỦI RO / KHÔNG ƯU TIÊN</h3>
<p class="full-note">Lấy từ bảng V13.4, đưa nhóm đỏ lên đầu để kiểm tra nhanh.</p>
{v134_red_top_html}
</div>
