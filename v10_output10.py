...
# ============================================================
# V10 PRO MAX FINAL - DEEP FILTER TOP CODES
# Display-only layer. Does NOT change core trading logic.
# Goals:
# - T+2: keep only top 3-5 codes
# - T+5: keep only top 3-5 codes
# - Rank by Avg return first, then OOS / AI / Score
# - Split pattern deeper by Regime + Strategy + RSI bucket + RS20 bucket
# ============================================================

def _pf_find_col(df, names):
    if df is None or df.empty:
        return None
    for n in names:
        if n in df.columns:
...
def _pf_pattern_parts(pattern_value):
    try:
...
def _pf_pattern_regime_strategy(pattern_value):
    parts = _pf_pattern_parts(pattern_value)
    reg = _pf_norm_regime(parts[0]) if len(parts) >= 1 else ""
    strat = _pf_norm_strategy(parts[1]) if len(parts) >= 2 else ""
    return reg, strat


def _pf_prepare_patterns(pattern_df, mode="T5"):
    if pattern_df is None or pattern_df.empty:
        return pd.DataFrame()

    pat = pattern_df.copy()
    pattern_col = _pf_find_col(pat, ["Pattern", "Pattern Key"])
    oos_col = _pf_find_col(pat, ["OOS%", "OOS Win Probability", "OOS Win Rate"])
    n_col = _pf_find_col(pat, ["OOSN", "OOS N", "OOS Samples"])
    ...
    if mode == "T2":
        min_oos, min_n, min_avg = 70, 5, 0.8
    else:
        min_oos, min_n, min_avg = 75, 5, 1.5
    ...
    if strict.empty:
        strict = pat[(pat["OOS%"] >= 70) & (pat["OOSN"] >= 3)].copy()

    if strict.empty:
        return pd.DataFrame()

    reg_str = strict[pattern_col].apply(_pf_pattern_regime_strategy)
    strict["PF Regime"] = reg_str.apply(lambda x: x[0])
    strict["PF Strategy"] = reg_str.apply(lambda x: x[1])
    strict["PF Key2"] = strict["PF Regime"].astype(str) + "|" + strict["PF Strategy"].astype(str)
    strict["Pattern Source"] = strict[pattern_col].astype(str)
    strict["Trade Mode"] = mode
    strict["Avg Type"] = avg_col
    strict["Avg Used"] = strict[avg_col]

    strict = strict.sort_values(["Avg Used", "OOS%", "OOSN"], ascending=False)
    return strict


def _pf_prepare_signals(signal_df):
    if signal_df is None or signal_df.empty:
        return pd.DataFrame()
    ...
    regime_col = _pf_find_col(sig, ["Market Regime Now", "Regime"])
    strategy_col = _pf_find_col(sig, ["Chien luoc", "Strategy", "Chiến lược"])

    sig["PF Regime"] = sig[regime_col].apply(_pf_norm_regime) if regime_col else ""
    sig["PF Strategy"] = sig[strategy_col].apply(_pf_norm_strategy) if strategy_col else ""

    sig["PF Key2"] = sig["PF Regime"].astype(str) + "|" + sig["PF Strategy"].astype(str)
    sig["RSI Bucket"] = sig["RSI"].apply(_pf_bucket_rsi) if "RSI" in sig.columns else "RSI_NA"
    sig["RS20 Bucket"] = sig["RS20"].apply(_pf_bucket_rs20) if "RS20" in sig.columns else "RS20_NA"

    sig["Deep Pattern"] = (
        sig["PF Regime"].astype(str) + "|" +
        sig["PF Strategy"].astype(str) + "|" +
...
def build_top_codes_by_proven_pattern_stable(signal_df, pattern_df, mode="T5", limit=5):
    sig = _pf_prepare_signals(signal_df)
    pat = _pf_prepare_patterns(pattern_df, mode=mode)
    ...
    if pat.empty:
        ...
        merged = sig.merge(
            pat.drop_duplicates("PF Key2"),
            on="PF Key2",
            how="left",
            suffixes=("", "_hist")
        )

        no_match = merged["OOS%"].isna() if "OOS%" in merged.columns else pd.Series([True] * len(merged), index=merged.index)
        if no_match.any():
            best = pat.iloc[0].to_dict()
            for k, v in best.items():
                if k not in merged.columns:
                    merged[k] = np.nan
                merged.loc[no_match, k] = v
            merged.loc[no_match, "Match Level"] = "GLOBAL_FALLBACK"
        merged.loc[~no_match, "Match Level"] = "REGIME+STRATEGY"
    ...
    if mode == "T2":
        if "RS20" in merged.columns:
            merged = merged[merged["RS20"].fillna(-999) > 0].copy()
