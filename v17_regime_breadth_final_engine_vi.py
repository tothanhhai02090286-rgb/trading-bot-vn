...
FIX HIỂN THỊ TELEGRAM:
1) Tổng quan V17
2) Hiển thị đủ toàn bộ mã nền V17
3) TOP tốt nhất trong chính danh sách nền V17
4) Kết luận cuối: dashboard TOP MUA THẬT + TOP THEO DÕI mới là bản chốt mua thật

Không đổi logic lọc/chấm điểm/output chính.
"""
...
def _short_reason(text, max_len=95):
    s = str(text or "")
    parts = [p.strip() for p in s.split("|") if p.strip()]
    s = " | ".join(parts[:4]) if parts else s
    if len(s) > max_len:
        s = s[:max_len-3] + "..."
    return s


def _line_for_stock(r, compact=False):
    ma = r.get("Mã", "")
    decision = r.get("Quyết định V17", "")
    score = fmt(r.get("Điểm V17"))
    action = r.get("Action gốc", "")
    strategy = r.get("Strategy V17", "")
    risk = r.get("Risk", "")
    reason = _short_reason(r.get("Lý do V17", ""), 110 if compact else 140)
    if compact:
        return f"• <b>{ma}</b> | {decision} | {score} | {strategy} | {risk} | {action}"
    return f"• <b>{ma}</b> | {decision} | Điểm {score}\n  {strategy} | Risk {risk} | Action {action}\n  Lý do: {reason}"


def report_text(df, source, market):
    lines = [
        "✅ <b>V17 REGIME + BREADTH FINAL HOÀN TẤT</b>",
        "",
        "<b>1. TỔNG QUAN V17</b>",
        f"Nguồn nền: <b>{source}</b>",
        f"Tổng mã nền V17: <b>{len(df)}</b>",
        f"Market: <b>{market['market_mode']}</b> | Score: <b>{market['market_score']}</b>",
        f"Above MA20: <b>{market['breadth_above_ma20_pct']}%</b> | MA5&gt;MA20: <b>{market['breadth_ma5_gt_ma20_pct']}%</b>",
        "",
        "<b>2. TOÀN BỘ MÃ NỀN V17</b>",
        "Mã | Quyết định | Điểm | Strategy | Risk | Action",
    ]
    if df.empty:
        lines.append("Không có mã nền.")
    else:
        for _, r in df.iterrows():
            lines.append(_line_for_stock(r, compact=True))
    lines += [
        "",
        "<b>3. TOP TỐT NHẤT TRONG DANH SÁCH NỀN V17</b>",
        "Ưu tiên: không LOẠI, Risk PASS, điểm cao, quyết định tốt.",
    ]
    if df.empty:
        best = pd.DataFrame()
    else:
        tmp = df.copy()
        tmp["_risk_pass"] = tmp["Risk"].astype(str).str.upper().str.contains("PASS", na=False)
        tmp["_is_reject"] = tmp["Quyết định V17"].astype(str).str.contains("LOẠI", na=False)
        tmp["_score"] = pd.to_numeric(tmp["Điểm V17"], errors="coerce").fillna(0)
        best = tmp[~tmp["_is_reject"]].copy()
        if best["_risk_pass"].any():
            best = best[best["_risk_pass"]].copy()
        best = best.sort_values(["_score"], ascending=False).head(10)
    if best.empty:
        lines.append("Không có mã top phù hợp.")
    else:
        for _, r in best.iterrows():
            lines.append("")
            lines.append(_line_for_stock(r, compact=False))
    lines += [
        "",
        "<b>4. KẾT LUẬN CUỐI</b>",
        "Bản quyết định mua thật xem tại:",
        "<b>TOP MUA THẬT + TOP THEO DÕI</b> trong dashboard.",
        "Render watchlist đã lấy từ 2 bảng này để canh realtime.",
        "",
        "Gợi ý đọc:",
        "• V17 Regime = danh sách nền rộng để tự so sánh.",
        "• TOP tốt nhất V17 = nhóm mạnh nhất trong nền V17.",
        "• Dashboard TOP MUA THẬT/THEO DÕI = bản chốt sau lọc sâu hơn.",
    ]
    return "\n".join(lines)
...
def send_telegram(text):
    ...
    try:
        ...
        for i, chunk in enumerate(chunks, 1):
            prefix = f"<b>V17 REPORT ({i}/{len(chunks)})</b>\n\n" if len(chunks) > 1 else ""
            r = requests.post(url, data={"chat_id": chat_id, "text": prefix + chunk, "parse_mode": "HTML", "disable_web_page_preview": True}, timeout=20)
...
<div class='card'><h3>4. BẢNG ĐẦY ĐỦ V17</h3>{df.to_html(index=False, escape=True)}</div>
    ...
    print(rep.replace("<b>", "").replace("</b>", "").replace("&gt;", ">"), flush=True)
